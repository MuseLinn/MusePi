...
export interface GuestSnapshot {
	...
	working: boolean;
	/** auto_compaction in flight (start event without its end) — suppresses
	 *  the chat-level working indicator; the compaction notice owns the wait
	 *  feedback (design doc §B judgment table). */
	compacting: boolean;
...
export class GuestClient {
	...
	#working = false;
	/** auto_compaction_start seen without its end event — while true the
	 *  chat-level working indicator is suppressed (the compaction notice
	 *  owns the wait feedback; design doc §B). */
	#compacting = false;
