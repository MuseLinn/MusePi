# @musepi/client-core
Client UI core for MusePi — the shared transcript/chat renderer consumed by the desktop GUI (`@musepi/desktop-app`), served as the remote-collab web client (paste a `/collab` link and you get the same live session guests see in the TUI), and packaged by the Capacitor Android app. Renamed from `@musepi/guest-client` 2026-09-21: "guest" no longer describes its role now that the desktop renderer imports it as the shared message-rendering layer.
...
- `src/lib/` — vendored wire codec (`codec.ts` AES-256-GCM, `link.ts` envelope + link grammar), `socket.ts` reconnecting relay socket, `client.ts` guest session store (`GuestClient` + immutable snapshots for `useSyncExternalStore`). Shared protocol shapes come from `@musepi/pi-wire`.
...
The package is intentionally standalone — no dependency on `@musepi/pi-coding-agent` at runtime or type level. Wire-shape drift is prevented by consuming the same `@musepi/pi-wire` contracts as the host, with sealed-frame interop still covered by `test/codec.test.ts`.
