...
const msg = (message: object): SessionEntry => ({ ...base, type: "message", message }) as unknown as SessionEntry;
...
describe("transcriptNodeKind dispatch", () => {
	test("message entries dispatch by role", () => {
		...
		expect(
			transcriptNodeKind(msg({ role: "toolResult", toolCallId: "c1", content: [], isError: false, timestamp: 1 })),
		).toBe("message:tool_result");
		expect(
	...
	test("non-message entry types dispatch on their own discriminant", () => {
		...
		expect(transcriptNodeKind({ ...base, type: "branch_summary", fromId: "e0", summary: "s" } as SessionEntry)).toBe(
			"branch_summary",
		);
		expect(transcriptNodeKind({ ...base, type: "model_change", model: "x/y" } as SessionEntry)).toBe("model_change");
		expect(
