...
export class AssemblySessionState {
	...
	get manifest(): AssemblyManifest | null {
	...
	get lastVerify(): AssemblyVerifyReport | null {
		return this._lastVerify;
	}
 ...
 */
export async function loadAssembly(cwd: string, home: string = os.homedir()): Promise<AssemblyLoadResult> {
	const manifest = await loadAssemblyManifest(cwd, home);
	const manifestPath = manifest ? findResolvedManifestPath(cwd, home) : null;
