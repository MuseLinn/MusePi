...
import type { AssemblyManifest, AssemblyVerifyReport, AssemblyExtensionError, Surface } from "./types.ts";
...
export function filterExtensionPaths(
  ...
  if (!manifest || ((manifest.extensions.include?.length ?? 0) === 0 && (manifest.extensions.exclude?.length ?? 0) === 0 && !manifest.extensions.patterns?.length)) {
