...
function normalizeManifest(raw: Record<string, unknown>): AssemblyManifestValidateResult {
  ...
  if (rawSurface !== undefined) {
    if (isSurface(rawSurface)) {
      ...
      validate("assembly.surface", `expected one of ${KNOWN_SURFACES.join(", ")}, got ${JSON.stringify(rawSurface)}`);
    }
  }
  ...
  if (rawTerminal.provider !== undefined) {
    if (isTerminalProvider(rawTerminal.provider)) {
      ...
      validate("seams.terminal.provider", `expected one of ${KNOWN_TERMINAL_PROVIDERS.join(", ")}, got ${JSON.stringify(rawTerminal.provider)}`);
    }
  }
  ...
  let compactionMethod: "handoff" | "remote" | "shake" | "snapcompact" | "soft" | undefined = undefined;
  if (rawCompaction.method !== undefined) {
    if (isCompactionMethod(rawCompaction.method)) {
      compactionMethod = rawCompaction.method;
    } else {
      validate("seams.compaction.method", `unknown compaction method — call musepi assembly status to see valid methods`);
    }
  }
  ...
  const manifest: AssemblyManifest = {
    ...
    seams: { terminal: terminalProvider !== undefined ? { provider: terminalProvider } : undefined, compaction: compactionMethod !== undefined ? { method: compactionMethod } : undefined },
  };
...
export async function loadAssemblyManifest(cwd: string, home: string): Promise<AssemblyManifest | null> {
  ...
  const raw = projectRaw !== null && globalRaw !== null
    ? mergeRaw(globalRaw, projectRaw)
    : projectRaw ?? globalRaw;
  if (!raw) return null;
