...
export class TasksBrowserComponent extends Container {
	...
	constructor(props: TasksBrowserProps, theme: any) {
		...
		this.props = props;
		this.theme = theme;
		this.sortedVisible = [...props.tasks]
			.filter((t) => props.filter === "all" || !isTerminal(t.status))
			.sort(compareTasks);
	...
	setProps(next: TasksBrowserProps): void {
		this.props = next;
		this.sortedVisible = [...next.tasks]
			.filter((t) => next.filter === "all" || !isTerminal(t.status))
			.sort(compareTasks);
		this.syncSelectionFromProps();
	...
	private matchKey: KeyMatchFn = (data, keyId) => {
		if (keyId.startsWith("tui.")) {
			try {
				if (this.keybindings?.matches(data, keyId as any)) return true;
			} catch {
			...
			try {
				return matchesKey(data, legacy as any);
			} catch {
				return false;
			}
		}
		try {
			return matchesKey(data, keyId as any);
		} catch {
			return false;
		}
	};
	...
	override render(width: number): string[] {
		...
		for (let i = 0; i < bodyHeight; i++) {
			inner.push((listFrame[i] ?? " ".repeat(listWidth)) + (rightFrames[i] ?? " ".repeat(rightWidth)));
		}
		inner.push(footer);
	...
	private renderFrame(title: string, content: readonly string[], width: number, height: number): string[] {
		...
		for (let i = 0; i < innerHeight; i++) {
			...
			lines.push(
				styledFg(this.theme, "accent", "│") + fitExactly(inner, innerWidth) + styledFg(this.theme, "accent", "│"),
			);
		}
		lines.push(bottom);
		return lines;
	...
	private renderListFrame(width: number, height: number): string[] {
		...
		if (this.sortedVisible.length === 0) {
			const empty = this.props.filter === "active" ? "No active tasks. Tab = show all." : "No tasks in this swarm.";
	...
	private renderListRow(task: SubAgentTask, selected: boolean, innerWidth: number): string {
		...
		const idText = selected ? styledBoldFg(this.theme, idColor, task.id) : styledFg(this.theme, idColor, task.id);
	...
	private renderTooSmall(width: number, rows: number): string[] {
		const lines: string[] = [];
		const msg = styledFg(this.theme, "error", `Terminal too small (need ≥ ${MIN_WIDTH} × ${MIN_HEIGHT})`);
