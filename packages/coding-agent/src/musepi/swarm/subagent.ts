...
function combineAbortSignals(signals: AbortSignal[]): { signal: AbortSignal; cleanup: () => void } {
	...
	for (const s of signals) {
		...
		offs.push(() => {
			try {
				s.removeEventListener("abort", onA);
			} catch {}
		});
	}
	return {
...
export function createSubagentResourceLoader(ctx: {
	...
	const enrichedPrompt = goalInjection ? `${systemPrompt}\n\n---\n${goalInjection}` : systemPrompt;
...
export async function retryOnRateLimit<T>(fn: () => Promise<T>, maxRetries = 10): Promise<T> {
...
export async function runSubAgent(
	...
	try {
		void hookEngine.fire(
			"SubagentStart",
			{ subagent_type: task.type, task_id: task.id },
			{ matcherText: task.type, cwd: ctx.cwd },
		);
	} catch {
		/* hooks fail open */
	}
	...
	try {
		...
		try {
			void hookEngine.fire(
				"SubagentStop",
				{ subagent_type: task.type, task_id: task.id, status: task.status },
				{ matcherText: task.type, cwd: ctx.cwd },
			);
		} catch {
			/* hooks fail open */
...
async function runWithModel(
	...
	const cleanupAbortListeners = () => {
		try {
			if (childController && onAbort) childController.signal.removeEventListener("abort", onAbort);
		} catch (e) {
			log_swarm_warn("removeEventListener(onAbort) failed", e);
		}
		try {
			if (combinedSignal && onCombinedAbort) combinedSignal.removeEventListener("abort", onCombinedAbort);
		} catch (e) {
			log_swarm_warn("removeEventListener(onCombinedAbort) failed", e);
		}
		try {
			unlink?.();
		} catch (e) {
			log_swarm_warn("unlink failed", e);
		}
		try {
			cleanupCombined();
		} catch (e) {
			log_swarm_warn("cleanupCombined failed", e);
		}
	};
	try {
		...
		try {
			progressEstimator.markStarted(task.id, Date.now());
		} catch (e) {
			log_swarm_warn("progressEstimator.markStarted failed", e);
		}
		...
		try {
			...
			if (wasTimeout) {
				...
				task.progressPercent = 100;
				try {
					progressEstimator.markFailed(task.id, Date.now());
				} catch (e) {
					log_swarm_warn("progressEstimator.markFailed failed", e);
				}
				onProgress();
		...
		if (allText.length > 0) {
			...
			task.outputLines =
				allText.length > MAX_OUTPUT_LINES
		...
		if (task.status === "done" || task.status === "failed") {
			try {
				...
				log_swarm_warn("progressEstimator final-mark failed", e);
			}
		}
