...
import { sanitizeShellOutput } from "@musepi/core/shell-output.js";
import {
	...
	serializeTask,
} from "@musepi/core/task/state.js";
import { createAgentSession } from "../../core/sdk.ts";
import { SessionManager } from "../../core/session-manager.ts";
import { createSubagentResourceLoader } from "../swarm/subagent.ts";

export type { BackgroundTaskEntry } from "@musepi/core/task/state.js";
...
class BackgroundTaskManager {
	...
	complete(taskId: string, outputLines: string[]): void {
		...
		try {
			void hookEngine.fire(
				"Notification",
				{ notification_type: "task.completed", message: `Background task ${taskId} completed` },
				{ matcherText: "task.completed" },
			);
		} catch {
			/* hooks fail open */
		}
	...
	fail(taskId: string, error: string, stopReason?: string): void {
		...
		try {
			void hookEngine.fire(
				"Notification",
				{ notification_type: "task.failed", message: `Background task ${taskId} failed: ${error}` },
				{ matcherText: "task.failed" },
			);
		} catch {
			/* hooks fail open */
		}
	...
	async stop(taskId: string, reason?: string): Promise<boolean> {
		...
		try {
			void hookEngine.fire(
				"Notification",
				{ notification_type: "task.aborted", message: `Background task ${taskId} stopped: ${normalized}` },
				{ matcherText: "task.aborted" },
			);
		} catch {
			/* hooks fail open */
		}
...
export function registerBackgroundTools(pi: any): void {
	...
	pi.registerTool({
		...
		description:
			"Run a subagent as a background task. Returns immediately. Use task_list/task_output to check results.",
		...
		parameters: {
			...
			properties: {
				...
				output_path: {
					type: "string",
					description:
						"Optional file path. When set, the full task output is written to this file on completion (use Read with offset/limit to page through large outputs).",
				},
			},
			required: ["prompt"],
		},
	...
	pi.registerTool({
		...
		async execute(_toolCallId: string, params: any, _signal: any, _onUpdate: any, _ctx: any) {
			...
			if (tasks.length === 0) {
				return {
					content: [
						{ type: "text", text: params?.active_only ? "No running background tasks." : "No background tasks." },
					],
				};
			}
			const lines = tasks.map((t) => {
				const dur = t.startTime
					? t.endTime
						? `${Math.floor((t.endTime - t.startTime) / 1000)}s`
						: "running"
					: "—";
	...
	pi.registerTool({
		...
		parameters: {
			...
			properties: {
				...
				offset: {
					type: "number",
					description: "1-based start line for paging (optional; offset/limit take precedence over tail)",
				},
				limit: {
					type: "number",
					description: "Max lines to return when paging (optional; offset/limit take precedence over tail)",
				},
				block: {
					type: "boolean",
					default: false,
					description: "Wait for the task to finish before returning output",
				},
		...
		async execute(_toolCallId: string, params: any, _signal: any, _onUpdate: any, _ctx: any) {
			...
			return {
				content: [
					{
						type: "text",
						text: ok ? `Task ${params.task_id} stopped.` : `Task ${params.task_id} not found or not running.`,
					},
				],
			};
		},
	...
	try {
		...
		unsub = session.subscribe((event: any) => {
			if (event.type === "message_end" && event.message?.role === "assistant") {
				...
				if (texts.length > 0) {
					backgroundManager.appendOutput(
						taskId,
						texts.map((p: any) => p.text || ""),
					);
				}
			}
		});
		...
		try {
			void hookEngine.fire(
				"SubagentStop",
				{ subagent_type: subagentType, task_id: taskId, status: backgroundManager.get(taskId)?.status },
				{ matcherText: subagentType },
			);
		} catch {
			/* hooks fail open */
