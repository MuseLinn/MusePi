// ============================================================
// Ask — interactive question dialog (pi-tui component).
//
// Ported from pi-muselinn-harness/ask/dialog.ts.
// One component drives the whole question set: a tab strip on top
// when there is more than one question, numbered options with
// description sub-lines, checkbox semantics for multi_select,
// and a synthetic Other option that opens a free-text input.
// Digit keys 1-9 jump straight to an option; Esc cancels everything.
// Answering the last open question lands on a submit page.
// ============================================================
...
class SideBySide implements Component {
	...
	constructor(left: string[], right: string[], leftWidth: number, gap: number) {
		this.left = left;
		this.right = right;
...
export class QuestionDialogComponent extends Container {
	...
	constructor(specs: QuestionSpec[], theme: any, onDone: (result: QuestionsDialogResult) => void) {
		super();
		this.specs = specs;
		this.theme = theme;
		this.onDone = onDone;
	...
	private renderSideBySide(st: AnswerState): void {
		...
		if (st.editingNote) {
			this.body.addChild(
				new Text(theme.fg("dim", `  note for ${st.spec.options[st.noteTarget]?.label ?? ""}:`), 1, 0),
			);
			this.noteInput.focused = true;
	...
	private previewBlock(st: AnswerState, width: number): string[] {
		...
		if (target < 0) {
			target = st.spec.options.findIndex((o) => typeof o.preview === "string" && o.preview.trim() !== "");
		}
		let lines: string[];
		if (target < 0) {
	...
	private renderSubmitPage(): void {
		...
		for (let i = 0; i < this.specs.length; i++) {
			...
			this.body.addChild(new Text(theme.fg("dim", `Q${i + 1} `) + theme.fg("text", spec.question), 1, 0));
			this.body.addChild(new Text(theme.fg("accent", "  → ") + val + noteMark, 1, 0));
		}
		this.body.addChild(new Spacer(1));
		this.body.addChild(new Text(theme.fg("dim", "Enter submit · ←/→ back to questions · Esc cancel"), 1, 0));
	}
	...
	private tabLine(): string {
		...
		parts.push(this.isSubmitPage() ? theme.fg("accent", theme.bold("Submit")) : theme.fg("dim", "Submit"));
