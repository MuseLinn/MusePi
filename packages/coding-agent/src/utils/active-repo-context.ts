...
function resolveRepository(cwd: string): string | null {
	try {
		return execSync("git rev-parse --show-toplevel", { cwd, encoding: "utf-8" }).toString().trim();
	} catch {
		return null;
	}
}

function resolveRepositorySync(cwd: string): string | null {
	try {
		return execSync("git rev-parse --show-toplevel", { cwd, encoding: "utf-8" }).toString().trim();
	} catch {
		return null;
...
export async function resolveActiveRepoContext(cwd: string): Promise<ActiveRepoContext | null> {
	const resolvedCwd = path.resolve(cwd);
	if (resolveRepository(resolvedCwd)) return null;
