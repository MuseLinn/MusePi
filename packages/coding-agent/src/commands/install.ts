/**
 * `musepi install <target>` — top-level convenience over `musepi plugin install` /
 * `musepi plugin link`.
 *
 * The docs (omp.sh/docs/extension-authoring) advertise
 *
 *   musepi install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `musepi install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: musepi install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
