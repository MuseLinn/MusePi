...
/**
 * `musepi ttsr` — inspect and test Time-Traveling Stream Rules.
 *
 * `musepi ttsr test` feeds a snippet (inline, --file, or stdin) through the real
 * TTSR matching pipeline and reports which rules would trigger. `musepi ttsr list`
...
export default class Ttsr extends Command {
	...
	static examples = [
		"musepi ttsr list",
		"musepi ttsr test 'const x: any = 1'",
		"musepi ttsr test src/foo.ts",
		"musepi ttsr test --file src/foo.ts",
		"musepi ttsr test --file src/foo.ts --source text",
		"musepi ttsr test --rule .omp/rules/no-any.md --source tool --path src/foo.ts 'const x: any = 1'",
		"echo 'Box::leak(&mut v)' | musepi ttsr test --file - --path src/lib.rs",
		"musepi ttsr test --source tool --tool edit --path src/foo.ts 'const x: any = 1'",
		"musepi ttsr scan",
		"musepi ttsr scan src/",
		"musepi ttsr scan -r .omp/rules/no-any.md src/",
	];
	...
	async run(): Promise<void> {
		...
		// inline text — so `musepi ttsr test src/foo.ts` works without --file.
