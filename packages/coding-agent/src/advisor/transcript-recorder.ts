...
import type { Message, UserMessage } from "@musepi/pi-ai";
import { SessionManager } from "../core/session-manager.ts";
import { debug } from "../utils/pi-logger.ts";
...
export class AdvisorTranscriptRecorder {
	...
	#queue: Promise<void>;
	readonly #resolveSessionFile: () => string | undefined;
	 ...
	 */
	constructor(
		resolveSessionFile: () => string | undefined,
		resolveCwd: () => string,
		filename: string = ADVISOR_TRANSCRIPT_FILENAME,
		after?: Promise<unknown>,
	) {
		this.#resolveSessionFile = resolveSessionFile;
	...
	record(message: AgentMessage): void {
		...
		switch (message.role) {
			...
			case "user":
				...
				persisted = { ...(message as UserMessage), synthetic: true, attribution: "agent" } as Message;
				break;
			default:
				return;
		}
		...
		this.#enqueue(async () => {
			if (file !== this.#file) {
				...
				this.#manager = SessionManager.open(file, undefined, cwd);
				this.#file = file;
			}
	...
	flush(): Promise<void> {
		return this.#enqueueResult(async () => {
			// SessionManager writes are synchronous; no explicit flush needed.
		});
	}
	...
	async #closeManager(): Promise<void> {
		...
		this.#file = undefined;
		// SessionManager uses sync file writes and holds no resources that need explicit closing.
		if (!manager) return;
	}

	#enqueue(work: () => Promise<void>): void {
		this.#queue = this.#queue.then(work, work).catch((err) => {
			debug("advisor transcript record failed", { err: String(err) });
		});
	}
