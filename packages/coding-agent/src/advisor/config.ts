...
import { BUILTIN_TOOL_NAMES, normalizeToolNames } from "../tools/builtin-names.ts";
import { warn as _warn } from "../utils/pi-logger.ts";
import { isEnoent } from "../utils/pi-utils.ts";
import { collectConfigCandidates } from "./watchdog.ts";

const logger = { warn: _warn };
...
interface ValidatedAdvisorEntry {
	name: string;
	model?: string;
	tools?: string[];
	instructions?: string;
...
interface ValidatedWatchdogYaml {
	instructions?: string;
	advisors?: ValidatedAdvisorEntry[];
}

/** Validate a parsed YAML value against the expected watchdog config schema. */
function validateWatchdogYaml(raw: unknown): ValidatedWatchdogYaml | null {
	if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
	const obj = raw as Record<string, unknown>;
	const result: ValidatedWatchdogYaml = {};
	...
	if (obj.advisors !== undefined) {
		if (!Array.isArray(obj.advisors)) return null;
		const advisors: ValidatedAdvisorEntry[] = [];
		for (const entry of obj.advisors) {
			if (!entry || typeof entry !== "object" || Array.isArray(entry)) return null;
			const e = entry as Record<string, unknown>;
			if (typeof e.name !== "string") return null;
			const advisor: ValidatedAdvisorEntry = { name: e.name };
			if (e.model !== undefined) {
				if (typeof e.model !== "string") return null;
				advisor.model = e.model;
			}
			if (e.tools !== undefined) {
				if (!Array.isArray(e.tools) || !e.tools.every((t) => typeof t === "string")) return null;
				advisor.tools = e.tools as string[];
			}
			if (e.instructions !== undefined) {
				if (typeof e.instructions !== "string") return null;
				advisor.instructions = e.instructions;
			}
			if (e.enabled !== undefined) {
				if (typeof e.enabled !== "boolean") return null;
				advisor.enabled = e.enabled;
			}
...
/**
 * Returns a stable session id for one advisor within one primary session.
 * Uses `crypto.randomUUID()` (UUIDv4) since Node.js has no native UUIDv7.
 *
...
export function getOrCreateAdvisorProviderSessionId(
	...
	slug: string,
	randomSessionId: () => string = () => randomUUID(),
): string | undefined {
	...
	if (!UUID_PATTERN.test(next)) {
		throw new Error("Advisor provider session id generator returned a non-UUID value");
	}
	ids.set(key, next);
	return next;
}
...
export async function discoverAdvisorConfigs(cwd: string, agentDir?: string): Promise<DiscoveredAdvisors> {
	...
	for (const item of items) {
		...
		const result = validateWatchdogYaml(parsed);
		if (!result) {
			logger.warn("Advisor config: invalid schema", { path: item.path, error: "validation failed" });
			continue;
		}
...
export async function resolveAdvisorConfigEditPath(
	...
	if (!(await fileExists(yml)) && (await fileExists(yaml))) return yaml;
	return yml;
}
...
export async function loadWatchdogConfigFile(filePath: string): Promise<WatchdogConfigDoc> {
	...
	const result = validateWatchdogYaml(parsed);
	if (!result) {
		logger.warn("Advisor config: invalid schema for edit", { path: filePath, error: "validation failed" });
		return { advisors: [] };
	}
...
/**
 * Quote a value as a YAML double-quoted scalar for inline use in hand-assembled
 * YAML output where document-level `stringify` would add an unwanted trailing
 * newline. Produces the same quoting as `JSON.stringify` which is valid YAML.
 */
 ...
 * delegates to JSON quoting (valid YAML double-quoted scalars). Round-trips
 * through {@link loadWatchdogConfigFile}. Returns `""` for an empty doc.
 */
...
export function serializeWatchdogConfig(doc: WatchdogConfigDoc): string {
	...
	if (doc.advisors.length > 0) {
		...
		for (const advisor of doc.advisors) {
			lines.push(`  - name: ${yamlQuote(advisor.name)}`);
			if (advisor.model?.trim()) lines.push(`    model: ${yamlQuote(advisor.model)}`);
