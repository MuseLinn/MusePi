...
export class SessionManager {
	...
	/**
	 * Physically drop every entry from `index` on (the entry at `index`
	 * included) and rewrite the session file. Used by revert/edit — the
	 * abandoned tail is gone, not branched. Returns the number of dropped
	 * entries.
	 */
	async truncateToIndex(index: number): Promise<number> {
		const kept = this.#entries.slice(0, index);
		const dropped = this.#entries.length - kept.length;
		if (dropped <= 0) return 0;
		this.#entries.length = 0;
		this.#entries.push(...kept);
		this.#index.rebuild(kept);
		this.#setLeaf(kept.length > 0 ? kept[kept.length - 1]!.id : null);
		await this.#rewriteAtomically();
	...
	static create(
		...
		manager.#resetToNewSession({ modeId });
		return manager;
	}
