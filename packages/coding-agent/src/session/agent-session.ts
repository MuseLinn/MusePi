...
import type { Clipboard, InMemorySnapshotStore } from "@musepi/hashline";
import {
...
import { type Effort, streamSimple } from "@musepi/pi-ai";
...
export class AgentSession {
	...
	async #emitSessionEvent(event: AgentSessionEvent, options?: { detachExtensions?: boolean }): Promise<void> {
		...
		try {
			await this.#emitExtensionEvent(event, options?.detachExtensions === true);
			await previousGate;
	...
	async #emitExtensionEvent(event: AgentSessionEvent, detach = false): Promise<void> {
		if (!this.#extensionRunner) return;
		// Detached emission (auto_compaction_end with detachPostCommit, etc.):
		// fire the extension handler without letting a hung handler pin the
		// mid-run loop — subscribers still get the event, extension observers
		// just resolve whenever their handler settles. Mirror the upstream
		// detachExtensions contract that the host bridge now forwards.
		// Turn-bookkeeping events (agent_start/turn_*) keep the inline path:
		// their handlers maintain `#turnIndex`, which must not run out of band.
		const detachable = event.type !== "agent_start" && event.type !== "turn_start" && event.type !== "turn_end";
		if (detach && detachable) {
			this.#emitExtensionEvent(event, false).catch(error => {
				logger.error("Detached extension emit failed", { eventType: event.type, error: String(error) });
			});
			return;
		}
		if (event.type === "agent_start") {
	 ...
	 * `device_id` is derived from both the persistent musepi install id and that
	...
	async resetSessionContext(): Promise<ResetSessionContextResult | undefined> {
		...
		// Drop the context-file cache and rebuild the base prompt so an edited
		// AGENTS.md (or other context file) is re-read from disk after /clear —
		// same cache-reset newSession() performs (upstream #9273).
		resetCapabilities();
	...
	/** The session's freeze gate (per-scope pause); side-session builders
	 *  (security coordinator) inherit it so a paused session's scan stays
	 *  frozen. Absent ⇒ process-global gate semantics. */
	get pauseGate(): PauseGate | undefined {
		return this.agent.pauseGate;
	}
	...
	/** Per-model effort ceiling (models with a cap disable higher ladder rungs). */
	get thinkingLevelCeiling(): ThinkingLevel | undefined {
	...
	async sendQueuedMessage(group: "steering" | "followUp", text: string): Promise<boolean> {
		...
		const index = queue.findIndex(m => isUserQueuedMessage(m) && queueChipText(m) === text);
		if (index < 0) return false;
		const rest = queue.filter((_, i) => i !== index);
		if (group === "steering") this.agent.replaceQueues(rest, [...this.agent.peekFollowUpQueue()]);
	...
	async newSession(options?: NewSessionOptions): Promise<boolean> {
		...
		try {
			...
			// discovery but would otherwise hit stale cached bytes (upstream #9273).
	 ...
	 */
	/**
	 * Refresh a lazy-loaded local model's runtime context window after its first
	 * real inference. llama.cpp's `meta.n_ctx` and LM Studio's
	 * `loaded_context_length` only appear once the model is actually loaded;
	 * discovery snapshots the architecture maximum, so patch it to what the
	 * backend serves (#3310/#3311/#9001).
	 *
	...
	async #refreshLazyLocalContext(model: Model): Promise<void> {
		try {
			...
			await this.#setModelWithProviderSessionReset(refreshed);
		} catch (error) {
	...
	async #reapplyExtendedContextPolicy(): Promise<void> {
		try {
			...
			logger.warn("extended-context reapply failed", { error: String(error) });
		}
	}
