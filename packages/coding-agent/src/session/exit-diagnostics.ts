...
export function createInterruptedTurnAbortMessage(
	...
	return {
		...
		stopReason: "aborted",
		errorMessage: `Previous MusePi process exited before completing the turn${causeText}. The turn was interrupted and will not continue automatically; send a new message to continue.`,
...
export function describePendingToolCalls(entries: readonly SessionEntry[]): string | undefined {
	...
	return `Previous session ended while ${pending.length} ${noun} remained pending: ${formatted}. The prior MusePi process exited before recording tool result(s).`;
}
