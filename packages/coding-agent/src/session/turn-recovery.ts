...
import { calculateRateLimitBackoffMs, parseRateLimitReason } from "@musepi/pi-ai";
import * as AIError from "@musepi/pi-ai/error";
import { kCursorExecResolved } from "@musepi/pi-ai/utils/block-symbols";
import { isFireworksFastModelId, toFireworksBaseModelId } from "@musepi/pi-catalog/fireworks-model-id";
import { modelsAreEqual } from "@musepi/pi-catalog/models";
import { extractRetryHint, logger, prompt } from "@musepi/pi-utils";
...
export class TurnRecovery {
	...
	promptAgentWithIdleRetry(
		...
		options?: { toolChoice?: ToolChoice; preEmittedMessages?: ReadonlySet<AgentMessage> },
	): Promise<void> {
	...
	classifyResolvedInterruptedToolTurn(message: AssistantMessage): "reasonless-abort" | "stream-stall" | undefined {
		...
		for (const block of message.content) {
			...
			// Fork customization: during a cursor stream stall the lazy watchdog
			// aborts the request signal and cursor.ts calls `h2Request.close()` —
			// unmarked server-side execs are not resolved yet and must not count.
			if (
				streamStall &&
	 ...
	 * resolves to. The second key is what makes a chain reachable from the end of
	 * another one — a session that lands on the last entry of one chain has no
	 * fallback of its own, but when that entry is itself a chain key with its own
	 * fallbacks configured, the second key finds them (PRs #8984/#9000). Two keys
	 * is the whole walk per attempt. Stale role hints are dropped upstream.
	 */
	...
	async #promptAgentWithIdleRetry(
		...
		for (;;) {
			try {
				await this.#host.agent.prompt(messages, { ...options, preEmittedMessages: options?.preEmittedMessages });
				return;
			} catch (err) {
