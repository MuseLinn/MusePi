 ...
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.musepi/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.musepi/auth-broker.token` (paired with URL from env or config).
 *
...
import { AuthBrokerError } from "@musepi/pi-ai/auth-broker";
import {
...
} from "@musepi/pi-ai/auth-broker/discover";
import { MissingApiKeyError } from "@musepi/pi-ai/error";
...
export function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const key = `${process.env.MUSEPI_AUTH_BROKER_URL ?? process.env.OMP_AUTH_BROKER_URL ?? ""}\u0000${process.env.MUSEPI_AUTH_BROKER_TOKEN ?? process.env.OMP_AUTH_BROKER_TOKEN ?? ""}\u0000${getAgentDir()}`;
