...
import { logger, prompt } from "@musepi/pi-utils";
...
export interface SecurityCoordinatorHost {
	...
	/** Freeze gate of the owning session (GUI per-session pause); the scan
	 *  reviewer session inherits it so a paused session's scan stays frozen.
	 *  Absent ⇒ the scan falls back to the process-global gate. */
	pauseGate?: PauseGate;
}
...
async function createDefaultSecuritySession(input: SecurityScanSessionFactoryInput): Promise<AgentSession> {
	...
	const { session } = await createAgentSession({
		...
		pauseGate: input.host.pauseGate,
...
export class SecurityCoordinator {
	...
	async #run(
		...
		try {
			...
			await reportProgress?.("Preparing MusePi-native security scan");
			...
			try {
				...
				await reportProgress?.("Reviewing repository with MusePi security workers");
