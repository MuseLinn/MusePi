...
import type { AgentTool, AgentToolResult, ToolTier } from "@musepi/pi-agent-core";
...
export class SecurityScanTool implements AgentTool<typeof securityScanSchema, SecurityScanToolDetails> {
	...
	readonly summary = "Run MusePi-native scans and explicit Codex Security cloud operations";
	...
	async execute(
		...
		const coordinatorForSession = () => {
			...
			return getSecurityCoordinator({
				...
				pauseGate: this.session.pauseGate,
			});
		};
