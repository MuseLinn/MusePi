...
import { isEnoent, isFsError, logger } from "@musepi/pi-utils";
 ...
 * wrapper.ts`), which wraps the whole tool registry whenever an `ExtensionRunner`
 * exists — so the component that owns the handlers is the one naming its own
 * session, and no caller has to thread an `AgentToolContext` through for
 * attribution to work.
 *
