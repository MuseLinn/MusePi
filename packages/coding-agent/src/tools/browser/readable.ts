import type * as DomNs from "@musepi/pi-utils/dom";
import type * as ReadabilityNs from "@musepi/pi-utils/readability";
...
async function loadReadability(): Promise<typeof ReadabilityNs> {
	if (!readabilityModule) {
		readabilityModule = await import("@musepi/pi-utils/readability");
	}
...
async function loadDom(): Promise<typeof DomNs> {
	if (!domModule) {
		domModule = await import("@musepi/pi-utils/dom");
	}
	return domModule;
}
