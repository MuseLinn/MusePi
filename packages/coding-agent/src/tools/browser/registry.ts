...
import { isCompiledBinary, logger, withTimeout, workerHostEntry } from "@musepi/pi-utils";
...
async function openBrowserHandle(kind: BrowserKind, opts: AcquireBrowserOptions): Promise<BrowserHandle> {
	...
	if (kind.kind === "headless") {
		// Every real musepi process (session, subagent, worker — anything with a CLI
...
async function disposeBrowserHandle(handle: BrowserHandle, opts: ReleaseBrowserOptions): Promise<void> {
	...
	if (handle.kind.kind === "headless") {
		if (handle.sharedDaemon) {
			...
			// daemon dies with the last musepi client in the project (broker idle
...
async function openSharedHeadlessHandle(
	...
	try {
		...
		if (!shared) {
			throw new ToolError(
				"Shared browser daemon unavailable (broker start or Chromium launch failed); check `hub ps` for musepi.browser.* daemons and ~/.musepi/logs for details",
			);
		}
		const puppeteer = await loadPuppeteer();
		const browser = await puppeteer.connect({
