...
import { $which, getPuppeteerDir, logger, removeWithRetries } from "@musepi/pi-utils";
import type * as BrowsersNs from "@musepi/pi-utils/browsers";
...
async function loadBrowsers(): Promise<typeof BrowsersNs> {
	if (!browsersModule) {
		browsersModule = await import("@musepi/pi-utils/browsers");
	}
