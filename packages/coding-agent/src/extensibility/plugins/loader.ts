...
import { getPluginsDir, getPluginsLockfile, isEnoent } from "@musepi/pi-utils";
 ...
 *
 * `home` controls which `<plugins>/musepi-plugins.lock.json` is read — pass it
 ...
 * `<root>/package.json#dependencies`, and `<root>/musepi-plugins.lock.json#plugins`.
...
async function collectPluginsAtRoot(
	...
	const lockPath = path.join(root, "musepi-plugins.lock.json");
 ...
 * `package.json#dependencies` and `musepi-plugins.lock.json#plugins`. Project
