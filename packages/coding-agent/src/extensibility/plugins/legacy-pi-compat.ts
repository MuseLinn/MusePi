...
import { isCompiledBinary, stripWindowsExtendedLengthPathPrefix } from "@musepi/pi-utils";
...
// fork, or the canonical @musepi scope itself) are remapped to this scope and
// resolved against the bundled copy that ships inside the musepi binary. This
...
const CANONICAL_PI_SCOPE = "@musepi";

// Scopes that have historically been used to publish (or alias) the same set
// of internal pi-* packages. `@musepi` is intentionally included so direct
...
const PI_SCOPE_ALIASES = ["oh-my-pi", "musepi", "mariozechner", "earendil-works"] as const;

// Internal pi-* package basenames bundled inside the musepi binary.
 ...
 * bare `@musepi/pi-coding-agent` here: from a global install Bun can pick an
...
// imports such as `@musepi/pi-ai/oauth` continue to resolve directly
...
/**
 * Resolve a canonical `@musepi/*` specifier to a filesystem path, preferring
...
function resolveLegacyPiSpecifier(args: { path: string; importer: string }): LegacyPiResolveResult | undefined {
	...
	// Primary: resolve the canonical @musepi/* specifier from the host binary
	...
	try {
		...
		// plugin installed @musepi peer deps, then try the original legacy
