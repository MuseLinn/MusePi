...
import type { ComputerSafetyCheck, ImageContent, Static, TextContent, TSchema } from "@musepi/pi-ai";
import { sanitizeText, untilAborted } from "@musepi/pi-utils";
	...
	implements AgentTool<TParameters, TDetails>
{
	...
	async execute(
		...
		try {
			...
			// that owns the handlers are both in scope (the whole tool registry is
			// wrapped with this class whenever a runner exists). Inert with no
			// fallback registered: no scope is entered.
			const raw = await withFileMutationSession(this.runner.sessionId || undefined, () =>
