...
import type { AssistantMessage, Context, ImageContent, Message, TextContent, VideoContent } from "@musepi/pi-ai";
import { compileSecretRegex } from "./regex.ts";
...
function generateDeterministicReplacement(secret: string): string {
	// Deterministic hash: use SHA256 first 8 bytes as BigInt
	const hashBuf = crypto.createHash("sha256").update(secret).digest();
	let hash = 0n;
	for (let i = 0; i < 8; i++) {
		hash = (hash << 8n) | BigInt(hashBuf[i]);
	}
	const chars: string[] = [];
	let h = hash;
...
function buildPlaceholder(index: number): string {
	// Deterministic 32-bit value from SHA256 first 4 bytes
	const buf = crypto.createHash("sha256").update(String(index)).digest();
	let v = 0;
	for (let i = 0; i < 4; i++) {
		v = (v << 8) | buf[i];
	}
...
// ═══════════════════════════════════════════════════════════════════════════
// Minimal SessionContext type for display restore
// ═══════════════════════════════════════════════════════════════════════════
...
type UserFacingMessage = Extract<Message, { role: "user" | "toolResult" }>;
...
function _deobfuscateTextBlocks(
	obfuscator: SecretObfuscator,
	content: (TextContent | ImageContent | VideoContent)[],
): (TextContent | ImageContent | VideoContent)[] {
	let changed = false;
