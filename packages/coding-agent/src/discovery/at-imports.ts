 ...
 * references inside their markdown memory files as inline includes. MusePi
...
import { debug as logDebug } from "../utils/pi-logger.ts";
...
async function resolveAndExpand(
	...
	if (visited.has(resolved)) {
		logDebug("@-import: skipping cyclic include", { path: resolved });
		return null;
	}

	const content = await readFile(resolved);
	if (content === null) {
		logDebug("@-import: file not found", { path: resolved });
		return null;
	}
...
/**
 * Read a file from the filesystem, returning null on any error
 * (ENOENT, permission, etc.).
 */
