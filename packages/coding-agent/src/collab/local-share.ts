 ...
 *
 * Both modes serve the guest-client `dist/` on the same port so the browser
...
const COLLAB_WEB_DIST = path.resolve(import.meta.dir, "../../../guest-client/dist");
...
export class LocalShareManager {
	...
	async startLan(): Promise<LanShareUrls> {
		...
		if (!this.webDistAvailable) {
			this.#onStatus?.("guest-client dist not built — browser UI unavailable, terminal join still works");
		}
