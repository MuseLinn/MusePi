...
export class OmfgPanelComponent extends Container {
	...
	#rebuild(): void {
		...
		this.addChild(new Text(theme.fg("accent", `/omfg ${this.#complaint}`), 1, 0));
		this.addChild(new Text(theme.fg("muted", this.#status), 1, 0));
		this.addChild(new Spacer(1));
	...
	#footerLine(): string {
		switch (this.#state) {
			...
			case "saved":
				return theme.fg(
					"success",
					`${getSymbol("status.success")} Registered live · ${this.#savedPath ?? "saved"} · Esc dismiss`,
				);
			case "rejected":
				return theme.fg("warning", `${getSymbol("status.warning")} Not saved · Esc dismiss`);
			case "aborted":
				return theme.fg("warning", `${getSymbol("status.warning")} Cancelled · Esc dismiss`);
			case "error":
				return theme.fg("error", `${getSymbol("status.error")} Error · Esc dismiss`);
		}
	}

	#contentComponent(): Component {
		if (this.#state === "error") {
			return new Text(theme.fg("error", this.#errorMessage ?? "Unknown error"), 1, 0);
		}
		const text = this.#preview.trim();
		if (!text) {
			return new Text(theme.fg("dim", `${getSymbol("status.pending")} Waiting for candidate rule…`), 1, 0);
		}
