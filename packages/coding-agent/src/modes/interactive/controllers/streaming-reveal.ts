...
import { formatThinkingForDisplay, hasDisplayableThinking } from "../../../utils/thinking-display.ts";
...
// ── LRU cache replacement: Map with manual eviction (max 128) ──────────────
...
type DisplayThinkingContentBlock = Extract<AssistantContentBlock, { type: "thinking" }> & {
	rawThinking?: string;
};
...
// ── Grapheme-counting utilities ────────────────────────────────────────────

const graphemeCountCache = new SimpleLRUCache<string, number>({ max: 128 });
