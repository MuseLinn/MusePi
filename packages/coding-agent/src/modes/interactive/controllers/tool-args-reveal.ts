...
/**
 * Minimum byte growth between throttled JSON re-parses of a streaming tool-call
 * argument buffer. A long write/edit/eval payload does not re-parse its full
 * JSON every frame — only when at least this many new bytes have accumulated.
 */
const STREAMING_JSON_PARSE_MIN_GROWTH = 1024;

/**
 * Lenient JSON parser that handles incomplete streaming JSON by progressively
 * fixing structural issues: trailing commas, unclosed brackets, and incomplete
 * string values. Returns the best-effort parse, falling back to an empty object.
 */
function parseStreamingJson<T>(text: string): T {
	...
	// If we ended inside a string value, truncate before it so we don't leave
	// a dangling colon with nothing on the right.
	let fixed = trimmed;
	if (inString) {
		inString = false;
		esc = false;
		...
		fixed = trimmed.slice(0, lastValidEnd);
	}

	// Remove trailing comma before each closer
	let result = fixed.replace(/,\s*$/, "");
...
/**
 * Throttled variant of `parseStreamingJson` that only re-parses when enough new
 * bytes have accumulated since the last parse (`STREAMING_JSON_PARSE_MIN_GROWTH`).
 * Returns `{ value, parsedLen }` on growth, or `undefined` to reuse prior state.
 */
function parseStreamingJsonThrottled<T>(
	text: string,
	lastParsedLen: number,
): { value: T; parsedLen: number } | undefined {
	if (text.length - lastParsedLen < STREAMING_JSON_PARSE_MIN_GROWTH) return undefined;
	return { value: parseStreamingJson<T>(text), parsedLen: text.length };
}
