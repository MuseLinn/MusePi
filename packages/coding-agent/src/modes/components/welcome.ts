...
/** Same tips localized; chosen at render time based on the active locale. */
const TIPS_ZH: readonly string[] = tipsZhText
	.split("\n")
	.map(line => line.trim())
	.filter(line => line.length > 0);
...
const NEW_TAG_TEXT = (): string => t("NEW!");
...
export function renderWelcomeTip(tip: string, boxWidth: number, phase = 0): string[] {
	...
	const body = t(isNew ? tip.replace(NEW_TIP_MARKER, "") : tip);
		...
		if (this.#selectedTip === undefined) {
			if (theme.getSymbolPreset() === "unicode" && Math.random() < 0.1) {
				...
				this.#selectedTip = pickWeightedTip(
					getLocale().toLowerCase().startsWith("zh") ? TIPS_ZH : TIPS,
					Math.random(),
				);
			}
		}
