...
import { getBlobsDir, isEnoent, logger, type postmortem, VERSION } from "@musepi/pi-utils";
import {
	type Agent,
...
import { loadAllExtensions } from "../../extensibility/extensions-center/state-manager";
	...
	async initialize(params: InitializeRequest): Promise<InitializeResponse> {
		...
		const authMethods: AuthMethod[] = [
			{
				...
				description: "Authenticate via the provider keys/OAuth state already configured under ~/.musepi.",
			},
		];
		if (params.clientCapabilities?.auth?.terminal === true) {
			authMethods.push({
				type: "terminal",
				id: "terminal",
				name: "Set up MusePi in terminal",
