...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@musepi/pi-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.musepi/, .claude/, .codex/, .gemini/)
	...
	// User config directories (~/.musepi/agent/, ~/.claude/, ~/.codex/, ~/.gemini/)
 ...
 * 2. Project config dirs: .musepi/lsp.*, .claude/lsp.*, .codex/lsp.*, .gemini/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.musepi/agent/lsp.*, ~/.claude/lsp.*, ~/.codex/lsp.*, ~/.gemini/lsp.* (+ hidden variants)
