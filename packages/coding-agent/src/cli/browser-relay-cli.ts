/**
 * `musepi browser-relay` implementation: serve the local CDP relay and install
...
export interface BrowserRelayCommandArgs {
	...
	port: number;
	token?: string;
	/** Install target directory; defaults to ~/.musepi/browser-relay/extension. */
	dir?: string;
...
async function runInstall(dirOverride: string | undefined): Promise<void> {
	...
	console.log(`Installed the MusePi Browser Relay extension to ${dir}`);
	console.log("");
...
async function runServe(args: BrowserRelayCommandArgs): Promise<void> {
	...
	try {
		...
		if (err instanceof Error && "code" in err && err.code === "EADDRINUSE") {
			if (await probeRelayServer(`http://127.0.0.1:${args.port}`)) {
				console.log(`musepi browser relay already running on http://127.0.0.1:${args.port}; nothing to do.`);
				return;
			}
			console.error(`Port ${args.port} is in use by something that is not an musepi browser relay.`);
			process.exit(1);
		}
		throw err;
	}
	...
	const readiness = setInterval(() => {
		if (relay.bridge.ready && !announced) {
			announced = true;
			console.log("Extension connected. The musepi browser tool can now drive your tabs.");
