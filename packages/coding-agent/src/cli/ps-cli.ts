/**
 * CLI handler for `musepi ps` — inspect and control processes supervised by the
 * daemon broker from outside the harness.
 *
 * A bare `musepi ps` on a TTY opens the interactive alt-screen monitor
...
import { truncateToWidth } from "@musepi/pi-tui";
import { formatDuration, getProjectDir } from "@musepi/pi-utils";
...
export async function runPsCommand(cmd: PsCommandArgs): Promise<void> {
	try {
		...
		if (!cmd.name) {
			console.error(chalk.red(`${cmd.action} requires a process name. Run \`musepi ps\` to list processes.`));
			process.exitCode = 1;
			return;
		}
