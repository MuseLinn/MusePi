 ...
 * `@musepi/pi-utils/env`, which eagerly loads `.env` from the agent directory
 ...
 * doesn't look like another flag. Without this, `musepi --system-prompt --profile
 ...
 * `musepi --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`musepi --print --profile work`).
 */
 ...
 * positionals are never stolen (`musepi grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `musepi launch --profile work` and
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `musepi --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`musepi --some-ext-bool --profile work`
		...
		// activates (`musepi --print --profile work`).
