/**
 * Interactive alt-screen monitor for `musepi ps` (btop idiom): a live process
 ...
 */
import { type Component, matchesKey, ProcessTerminal, TUI, truncateToWidth } from "@musepi/pi-tui";
import { formatDuration } from "@musepi/pi-utils";
...
class PsTopComponent implements Component {
	...
	#header(width: number, title: string): string {
		...
		const left = ` ${chalk.bold("musepi ps")} ${chalk.dim("·")} ${title}`;
