...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "omp") {
		throw new Error('Invalid alias "omp". Refusing to shadow the base musepi command.');
	}
...
function renderAliasBlock(
	...
	const start = `# >>> musepi profile alias: ${aliasName} >>>`;
	const end = `# <<< musepi profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
