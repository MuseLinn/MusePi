/**
 * Data layer shared by the `musepi ps` renderers (plain CLI and interactive
 * TUI): broker-scope discovery, daemon snapshot collection, and display cells.
 *
...
// ---------------------------------------------------------------------------
// musepi launch-path primitives (upstream `pi-utils`/`launch/paths` exposes
// these; musepi keeps them private, so ps re-derives them locally).
// ---------------------------------------------------------------------------
...
/** Parent of a project daemon runtime dir: the hash-keyed scope container. */
function daemonRuntimeRoot(): string {
	return path.dirname(getDaemonRuntimeDir("ps"));
}

/** Parent of a machine-global service runtime dir: the `global` scope container. */
function globalDaemonRuntimeRoot(): string {
	return path.dirname(getGlobalDaemonRuntimeDir("browser-relay"));
}
...
/**
 * Read the project directory recorded for a runtime dir; undefined when absent
 * or malformed. musepi's broker does not write `scope.json`, so this is a
 * best-effort read that normally returns undefined and falls back to the
 * client-presence recovery path below.
 */
async function readDaemonScopeMeta(runtimeDir: string): Promise<string | undefined> {
	try {
...
async function readLiveDaemonBrokerPid(runtimeDir: string): Promise<number | undefined> {
	let raw: unknown;
	try {
		raw = await Bun.file(path.join(runtimeDir, "broker.pid")).json();
	} catch {
...
export async function discoverScopes(): Promise<PsScope[]> {
	...
	for (const entry of await readdirQuiet(daemonRuntimeRoot())) {
		if (!entry.isDirectory() || !PROJECT_SCOPE_KEY.test(entry.name)) continue;
		const runtimeDir = path.join(daemonRuntimeRoot(), entry.name);
		scopes.push({
			kind: "project",
			runtimeDir,
	...
	for (const entry of await readdirQuiet(globalDaemonRuntimeRoot())) {
		if (!entry.isDirectory()) continue;
		const runtimeDir = await canonicalRuntimeDir(path.join(globalDaemonRuntimeRoot(), entry.name));
		scopes.push({
			kind: "global",
			runtimeDir,
			service: entry.name,
