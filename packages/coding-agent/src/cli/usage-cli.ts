 ...
 *
 * Handles `musepi usage` — fetches provider usage reports for every
...
import { formatDuration, formatNumber, sanitizeText } from "@musepi/pi-utils";
...
/** Human-readable disable cause: the upstream `error_description` when embedded, else the first clause. */
		...
		if (cmd.history) {
			...
			if (entries.length === 0) {
				...
				process.stderr.write(
					chalk.yellow(
						`No usage history recorded${scope} yet. Snapshots accumulate whenever usage is fetched (TUI footer, /usage, musepi usage).\n`,
					),
				);
				process.exitCode = 1;
				return;
