import type { ResolvedThinkingLevel } from "@musepi/pi-agent-core";
import type {
	Api,
	ApiKeyResolver,
	AssistantMessage,
...
import { resolveModelServiceTier, streamSimple } from "@musepi/pi-ai";
...
import { formatDuration, getProjectDir, prompt } from "@musepi/pi-utils";
...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	if (command.models.length === 0) {
		throw new Error("Pass at least one model selector, e.g. `musepi bench opus gpt-5.2`");
	}
