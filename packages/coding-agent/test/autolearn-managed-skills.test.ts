...
import {
	...
	toSkillFrontmatter,
	writeManagedSkill,
} from "@musepi/pi-coding-agent/autolearn/managed-skills";
import { parseFrontmatter, removeWithRetries } from "@musepi/pi-utils";
...
describe("managed-skills primitives", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "musepi-managed-skills-"));
