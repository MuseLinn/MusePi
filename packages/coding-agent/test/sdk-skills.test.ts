...
import { getActiveSkills } from "@musepi/pi-coding-agent/extensibility/skills";
import type { Skill } from "@musepi/pi-coding-agent/sdk";
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in the project skills dir for native project-level discovery
...
`,
	...
	it("SDK invocation root scope isolates disabled discovery and merges normal discovery", async () => {
		...
		const installedPackage = path.join(getPluginsDir(tempHomeDir), "node_modules", "sdk-installed-extension");
	...
	it("refreshSkills reloads project skills on an existing session", async () => {
		...
		const runtimeSkillDir = path.join(getProjectAgentDir(tempDir), "skills", "runtime-added-skill");
...
`,
	...
	it("manage_skill hot-registers managed skills in the active session", async () => {
		...
		const managedAgentDir = getAgentDir();
