 ...
 *
 * Note: helpers.ts imports @musepi/pi-natives (Rust addon via glob).
...
import type { InstalledPluginEntry } from "@musepi/pi-coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .musepi/ directory", async () => {
		// Layout: tmpDir/.musepi/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .musepi/ there.
		fs.mkdirSync(path.join(tmpDir, ".musepi"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".musepi", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .musepi/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.musepi/   +   tmpDir/sub/.musepi/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.musepi/, not climb further to tmpDir/.musepi/.
		fs.mkdirSync(path.join(tmpDir, ".musepi"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".musepi"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".musepi", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .musepi/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .musepi/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .musepi/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .musepi/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.musepi/.../installed_plugins.json.
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".musepi", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		// Create .musepi/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".musepi", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".musepi", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".musepi", "plugins", "installed_plugins.json");
	});
