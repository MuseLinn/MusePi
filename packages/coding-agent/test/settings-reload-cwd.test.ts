...
import { getProjectAgentDir, removeSyncWithRetries, Snowflake } from "@musepi/pi-utils";
	...
	describe("project layer (on disk)", () => {
		...
		it("keeps failed project writes bound to their original cwd", async () => {
			...
			try {
				...
				expect(await Bun.file(path.join(getProjectAgentDir(bareProject), "config.yml")).exists()).toBe(false);
			} finally {
				mkdirSpy.mockRestore();
			}

			await settings.flush();
			expect(YAML.parse(await Bun.file(path.join(getProjectAgentDir(startDir), "config.yml")).text())).toEqual({
		...
		it("writes project model roles only to the project YAML", async () => {
			const settings = await Settings.init({ cwd: startDir, agentDir });
			const projectConfigPath = path.join(getProjectAgentDir(startDir), "config.yml");
		...
		it("does not copy unedited roles from other project settings providers", async () => {
			...
			expect(YAML.parse(await Bun.file(path.join(getProjectAgentDir(scopedProject), "config.yml")).text())).toEqual({
