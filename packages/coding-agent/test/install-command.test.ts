...
import { commands, isSubcommand, resolveCliArgv } from "@musepi/pi-coding-agent/cli-commands";
import { looksLikeLocalPath } from "@musepi/pi-coding-agent/commands/install";
...
describe("install command is registered as a top-level subcommand", () => {
	...
	test("CLI runner rejects only bare reserved management words", () => {
		expect(resolveCliArgv(["extensions"])).toEqual({
			error: '`musepi extensions` is not a management command. Use `musepi plugin list` / `musepi plugin install`, or run `musepi launch extensions` if you meant to send "extensions" as a prompt.',
		});
...
describe("looksLikeLocalPath", () => {
	...
	test("npm specs and marketplace refs are remote", () => {
		expect(looksLikeLocalPath("@musepi/exa")).toBe(false);
