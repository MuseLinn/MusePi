...
import type { CursorExecHandlers } from "@musepi/pi-coding-agent/cursor";
import {
...
import { initializeExtensions } from "@musepi/pi-coding-agent/modes/runtime-init";
import {
...
} from "@musepi/pi-coding-agent/sdk";
...
import { logger, removeSyncWithRetries, Snowflake, untilAborted } from "@musepi/pi-utils";
...
describe("createAgentSession defaultInactive tool activation", () => {
	...
	afterAll(() => {
		// Windows cannot delete a directory while its SQLite DB is still open;
		// close the shared registry storage before removing it (posix unlinks
		// open files fine, win32 does not).
