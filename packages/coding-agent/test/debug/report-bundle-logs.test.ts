...
import { createReportBundle } from "@musepi/pi-coding-agent/debug/report-bundle";
import { getConfigRootDir, getLogsDir, removeWithRetries, setAgentDir } from "@musepi/pi-utils";
...
describe("report bundle logs", () => {
	it("collects every same-day PID log, not only the current process", async () => {
		...
		await fs.mkdir(path.join(xdgStateHome, "musepi"), { recursive: true });
		...
		const crashedName = `musepi.${today}.4242.log`;
		const rotatedName = `${crashedName}.1`;
		const currentName = `musepi.${today}.${process.pid}.log`;
