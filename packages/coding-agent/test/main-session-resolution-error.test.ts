...
import type { Args } from "@musepi/pi-coding-agent/cli/args";
import type { Settings } from "@musepi/pi-coding-agent/config/settings";
import { createSessionManager, SessionResolutionError, writeStartupNotice } from "@musepi/pi-coding-agent/main";
	...
	it("rejects --resume with SessionResolutionError carrying a usage hint", async () => {
		...
		try {
			await expect(
				...
				hint: expect.stringContaining("musepi --resume"),
			});
