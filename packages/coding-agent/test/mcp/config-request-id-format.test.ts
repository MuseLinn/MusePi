 ...
 *
 * Both OMP-native loaders are covered: `.musepi/mcp.json` (native provider) and a
...
import { getConfigRootDir, removeWithRetries, setAgentDir } from "@musepi/pi-utils";
...
test("requestIdFormat from .musepi/mcp.json reaches the transport config", async () => {
	const configs = await loadFrom(path.join(".musepi", "mcp.json"), {
