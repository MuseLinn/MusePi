...
import { describeAuthBrokerStartupError } from "@musepi/pi-coding-agent/session/auth-broker-config";
import { setInteractiveHost } from "@musepi/pi-utils";
...
describe("describeAuthBrokerStartupError", () => {
	it("turns a broker connection failure into recovery guidance", async () => {
		...
		expect(message).toContain("musepi auth-broker serve");
		expect(message).toContain("musepi config reset auth.broker.url");
...
describe("runRootCommand — unreachable auth broker at startup", () => {
	it("exits 1 with an actionable message instead of an uncaught AuthBrokerError", async () => {
		...
		expect(stderr).toContain("musepi auth-broker serve");
	}, 15_000);
});
