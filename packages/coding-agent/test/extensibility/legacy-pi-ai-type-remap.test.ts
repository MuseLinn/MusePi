...
import {
	...
	getBundledProviders,
	modelsAreEqual,
} from "@musepi/pi-catalog/models";
import { Type as TypeBoxShimType } from "@musepi/pi-coding-agent/extensibility/legacy-typebox";
import {
...
// pi-ai 15.1.0 removed the runtime `Type` export from `@musepi/pi-ai`'s
...
describe("legacy-pi @(scope)/pi-ai root `Type` remap (issue #1437)", () => {
	...
	it('redirects `import { Type } from "@musepi/pi-ai"` for plugins published against the canonical scope', async () => {
		const entry = await writeFixtureExtension(
			['import { Type } from "@musepi/pi-ai";', "export const probe = Type;"].join("\n"),
		);
	...
	it("does not redirect subpath imports such as @musepi/pi-ai/utils/schema", async () => {
		const entry = await writeFixtureExtension(
			[
	...
	it("exports getModel as getBundledModel", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension('import { getModel } from "@musepi/pi-ai"; export const testGetModel = getModel;'),
	...
	it("exports getModels as getBundledModels", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				'import { getModels } from "@musepi/pi-ai"; export const testGetModels = getModels;',
			),
		)) as { testGetModels: unknown };
		expect(loaded.testGetModels).toBe(getBundledModels);
	});

	it("re-exports calculateCost from @musepi/pi-catalog/models (issue #4584)", async () => {
		// `calculateCost` was moved from the `@musepi/pi-ai` barrel to
		// `@musepi/pi-catalog/models` in the catalog split. Legacy extensions
		...
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				'import { calculateCost } from "@musepi/pi-ai"; export const probe = calculateCost;',
			),
		)) as { probe: unknown };
		expect(loaded.probe).toBe(calculateCost);
	});

	it("re-exports modelsAreEqual and getBundledProviders from @musepi/pi-catalog/models", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				[
					'import { modelsAreEqual, getBundledProviders } from "@musepi/pi-ai";',
	...
	it("re-exports getBundledModel and getBundledModels from @musepi/pi-catalog/models", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				[
					'import { getBundledModel, getBundledModels } from "@musepi/pi-ai";',
...
describe("legacy pi package root remaps (issue #1474)", () => {
	it("loads @earendil-works/pi-coding-agent root imports when host package resolution is unavailable", async () => {
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@musepi/pi-coding-agent" && from.endsWith(path.join("src", "extensibility", "plugins"))) {
	...
	it("falls back to legacy-scoped subpath peers for direct plugin imports", async () => {
		const realResolveSync = Bun.resolveSync.bind(Bun);
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@musepi/pi-ai/oauth") {
	...
	it("routes @earendil-works/pi-utils through canonical Bun.resolveSync in non-compiled mode", async () => {
		...
		// `node_modules/@musepi/pi-*`, not next to the source tree. Hardcoding
