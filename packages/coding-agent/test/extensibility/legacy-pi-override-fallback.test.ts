...
describe("legacy pi compat package-root override validation (issue #2168)", () => {
	it("keeps overrides whose filesystem targets exist", () => {
		const candidates = {
			"@musepi/pi-ai": "/tmp/exists-ai.js",
			"@musepi/pi-utils": "/tmp/exists-utils.js",
		};
	...
	it("drops overrides whose filesystem targets are missing on disk", () => {
		const candidates = {
			"@musepi/pi-ai": "/tmp/exists-ai.js",
			"@musepi/pi-coding-agent": "/tmp/exists-shim.js",
			"@musepi/pi-utils": "/$bunfs/root/packages/utils/src/index.js",
			"@musepi/pi-tui": "/$bunfs/root/packages/tui/src/index.js",
		};
		...
		expect(result).not.toHaveProperty("@musepi/pi-utils");
		expect(result).not.toHaveProperty("@musepi/pi-tui");
	});
	...
	it("keeps virtual omp-legacy-pi-bundled: entries without touching the filesystem (issue #3423)", () => {
		...
		let probed = false;
		const candidates = {
			"@musepi/pi-ai": "omp-legacy-pi-bundled:@musepi/pi-ai",
			"@musepi/pi-coding-agent": "omp-legacy-pi-bundled:@musepi/pi-coding-agent",
			"@musepi/pi-agent-core": "omp-legacy-pi-bundled:@musepi/pi-agent-core",
			"@musepi/pi-natives": "omp-legacy-pi-bundled:@musepi/pi-natives",
			"@musepi/pi-tui": "omp-legacy-pi-bundled:@musepi/pi-tui",
			"@musepi/pi-utils": "omp-legacy-pi-bundled:@musepi/pi-utils",
		};
	...
	it("mixes virtual and filesystem entries: virtuals always pass, filesystems gated", () => {
		const candidates = {
			"@musepi/pi-ai": "omp-legacy-pi-bundled:@musepi/pi-ai",
			"@musepi/pi-coding-agent": "/dev/source/legacy-pi-coding-agent-shim.ts",
			"@musepi/pi-tui": "/missing/path.ts",
		};
