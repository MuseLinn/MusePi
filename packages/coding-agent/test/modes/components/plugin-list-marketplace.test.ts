...
import { PluginManager } from "@musepi/pi-coding-agent/extensibility/plugins";
import {
...
} from "@musepi/pi-coding-agent/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("musepi plugin install <package>");
		expect(text).toContain("musepi plugin install <name>@<marketplace>");
	});
