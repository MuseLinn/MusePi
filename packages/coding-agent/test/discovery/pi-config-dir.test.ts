...
import { getConfigDirs } from "@musepi/pi-coding-agent/config";
import { resolveClaudePaths } from "@musepi/pi-coding-agent/config/claude-paths";
import { getUserPath } from "@musepi/pi-coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		...
		expect(result[0]).toEqual({ path: expected, source: ".musepi", level: "user" });
	});
});
