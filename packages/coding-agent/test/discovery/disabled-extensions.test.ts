...
import { type ContextFile, contextFileCapability } from "@musepi/pi-coding-agent/capability/context-file";
import { resetSettingsForTest, Settings } from "@musepi/pi-coding-agent/config/settings";
import { initializeWithSettings, loadCapability } from "@musepi/pi-coding-agent/discovery";
...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		setAgentDir(path.join(tempHomeDir, ".musepi", "agent"));
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "omp-disabled-ext-"));
		await fs.mkdir(path.join(getProjectAgentDir(tempDir)), { recursive: true });
		await fs.writeFile(path.join(getProjectAgentDir(tempDir), "AGENTS.md"), "# project instructions\n");
