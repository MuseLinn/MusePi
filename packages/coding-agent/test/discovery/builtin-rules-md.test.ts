 ...
 * from both `~/.musepi/agent/RULES.md` (user) and the nearest `.musepi/RULES.md`
...
import { type Rule, ruleCapability } from "@musepi/pi-coding-agent/capability/rule";
import type { LoadContext } from "@musepi/pi-coding-agent/capability/types";
// Importing discovery registers all providers as a side effect.
import { loadCapability } from "@musepi/pi-coding-agent/discovery";
import { getConfigRootDir, removeSyncWithRetries, setAgentDir } from "@musepi/pi-utils";
...
test("user ~/.musepi/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".musepi", "agent", "RULES.md"),
...
test("project .musepi/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".musepi", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".musepi", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".musepi", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".musepi", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".musepi", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".musepi", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .musepi/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".musepi", "agent", "rules", "other.md"), "# Unrelated rule\n");
