...
import { createMCPTimeout, isMCPTimeoutEnabled, resolveMCPTimeoutMs } from "@musepi/pi-coding-agent/mcp/timeout";
import { logger } from "@musepi/pi-utils";

const ORIGINAL_TIMEOUT = process.env.MUSEPI_MCP_TIMEOUT_MS;

afterEach(() => {
	if (ORIGINAL_TIMEOUT === undefined) {
		delete process.env.MUSEPI_MCP_TIMEOUT_MS;
	} else {
		process.env.MUSEPI_MCP_TIMEOUT_MS = ORIGINAL_TIMEOUT;
	}
});
...
describe("MCP timeout configuration", () => {
	...
	test("allows the env override to disable MCP client-side timeouts", () => {
		process.env.MUSEPI_MCP_TIMEOUT_MS = "0";
	...
	test("allows the env override to set one timeout for every server", () => {
		process.env.MUSEPI_MCP_TIMEOUT_MS = "180000";

		expect(resolveMCPTimeoutMs(30_000)).toBe(180_000);
	});

	test("rejects negative env values and warns, falling back to the default", () => {
		process.env.MUSEPI_MCP_TIMEOUT_MS = "-1";
		const warn = spyOn(logger, "warn").mockImplementation(() => {});

		try {
			expect(resolveMCPTimeoutMs(120_000)).toBe(120_000);
			expect(warn).toHaveBeenCalledTimes(1);
			expect(warn.mock.calls[0]?.[0]).toContain("MUSEPI_MCP_TIMEOUT_MS");
		} finally {
			warn.mockRestore();
		}
	});
	...
	test("rejects non-numeric env values and falls back to the default", () => {
		process.env.MUSEPI_MCP_TIMEOUT_MS = "not-a-number";
		const warn = spyOn(logger, "warn").mockImplementation(() => {});
