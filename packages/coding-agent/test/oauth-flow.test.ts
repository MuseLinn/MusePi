...
import { MCPOAuthFlow, refreshMCPOAuthToken } from "@musepi/pi-coding-agent/mcp/oauth-flow";
...
describe("mcp oauth flow", () => {
	it("uses oh-my-pi client name for dynamic client registration", async () => {
		...
		expect((registrationPayload as { client_name?: string } | null)?.client_name).toBe("oh-my-pi");		expect((registrationPayload as { scope?: string } | null)?.scope).toBeUndefined();
