...
- **更新弹窗全流程中间态**:点击「下载更新」即进入 preparing 态(不确定进度条,覆盖 electron-updater 首个 download-progress 事件前的点击→首字节空窗,带重入保护防双击);下载中显示百分比 + 已传/总量 MB + 实时速度(`bytesPerSecond`);下载完成显示成功行 + 立即重启;被关掉的弹窗在 preparing/downloaded 推送时自动复活(`autoInstallOnAppQuit=false` 下立即重启是唯一安装路径);失败态新增重试按钮;notes 超 200 字符可展开/收起;关闭播 180ms 退出动画。设置页「检查更新」新增应用内下载按钮(同样经复活机制显示进度),notes 以 Markdown 渲染。
  - EN: Full intermediate states for the update toast: clicking Download update enters a preparing state (indeterminate bar covering the click→first-byte gap before electron-updater's first download-progress event, with a re-entry guard against double-clicks); downloading shows percent + transferred/total MB + live speed (`bytesPerSecond`); downloaded shows a success row + Restart now; a dismissed toast revives on preparing/downloaded pushes (with `autoInstallOnAppQuit=false`, Restart now is the only install path); a failed download gains a retry button; notes longer than 200 chars get a show-more toggle; dismissal plays a 180ms exit animation. The settings page's Check-for-updates row gains an in-app download button (progress surfaces via the same revival) and renders notes as Markdown.

### Fixed

- **CI runtime/session bucket 全绿修复(存量失败 + utils 回归)**:`issue-2750` fake session 缺 executor 的 `subscribeRunState`(TypeError 被吞进 result.error);`session-loader-stream` parity 暴露 utils `parseJsonlLenient` 丢了 `onMalformedRecord` 回调(恢复上游行为);`session-manager-immediate-persist` latch 断言只 mock `fs.writeSync` 漏了 rewrite 的 `writeFileSync`;`sdk-preloaded-extensions-isolation` runtime mock 缺 `pendingMediaProviderRegistrations`。
  - EN: Green the runtime/session CI bucket (stale failures + a utils regression): issue-2750 fake session missing executor's `subscribeRunState`; parseJsonlLenient had dropped its onMalformedRecord callback; the persist latch test mocked only fs.writeSync while the post-latch rewrite uses fs.writeFileSync; the preloaded-extensions runtime mock lacked pendingMediaProviderRegistrations.
- **UI/TUI bucket 两批存量失败修复(15 文件)**:editor/ctx mocks 缺产品演进后新增方法(`setCollapsedText`/`insertAtom`/`insertTextAttachment`/`getComposerShapes`/`syncComposerShape`/`resetDisplay`);vibe-toggle gate 错方法(小文件 loader 走 readText);job-poll displaceable 块非 finalized 是 musepi 设计;status-line usage 归一到 active model + context key 带 model id;transcript image marker 折叠为 chip(引用仍带 OSC-8 链接);品牌断言 omp→musepi。
  - EN: Two UI/TUI batches (15 files): mocks missing post-fork methods; vibe-toggle gated readTextSlices instead of readText; displaceable blocks staying unfinalized is musepi's design; status-line usage scoped to the active model with model id in the context key; transcript image markers collapse to chips (OSC-8 links preserved); brand assertions drifted omp→musepi.
- **native/unit bucket 存量失败修复(worker + 工具语义)**:`bun -e` 的 `@musepi/...` import 从 eval 上下文解析失败(改绝对路径);64KiB stderr 内联 `-e` argv 触发 ENAMETOOLONG(改临时文件);job-tool roster 用 session 佐证 isRunning 使无 session 的 running sub 消失(声明的 running 状态就该展示);export-html 硬编码字节基线停在旧 tool-views bundle(更新基线);markit PDF 路径无表格 pad(native pdf-inspector 输出 unpadded)。
  - EN: native/unit bucket stale failures: bun -e eval-context import resolution (absolute paths); 64KiB stderr in -e argv hit Windows ENAMETOOLONG (temp file); job-tool roster filtered on session-corroborated isRunning (claimed running status is what operators see); export-html hardened byte baseline pinned the old tool-views bundle (refreshed); the markit PDF path emits unpadded tables (native pdf-inspector bridge).
- **discovery/worker/registry 产品级修复**:`disableExtensionDiscovery` 现在也排除 installed-plugin skills(agent-plugins marketplace 扫描读 ALS root mode);`supportsExternalThinking` 对 reasoning-only Responses 模型(Grok 4 等)补 compat guard;`listXdevTools` 不把 `hidden:true` custom tools 列进 xd:// 展示;streaming edit preview 同 tick settle 取消 compute(微任务延迟 + previewDiffSettled 早退)。
  - EN: Product-level fixes: disableExtensionDiscovery now also excludes installed-plugin skills; supportsExternalThinking gained the reasoning-only compat guard; listXdevTools hides hidden:true custom tools; streaming edit previews cancel on same-tick settled results (microtask deferral + previewDiffSettled early return).
- **更新弹窗的更新内容预览从未显示**:发布 workflow 写入 update-manifest.json 的 `notes` 是纯字符串(整个 changelog 段落),而 daemon `updates.check` 只接受 `{zh,en}` 对象、字符串直接丢弃 → 弹窗只显示版本号。notes 改由 Electron 主进程直接拉取并缓存(`updater-notes` IPC,`checkForUpdates` 富结果同步携带),不再依赖 daemon 连接状态与 `startup.checkUpdate` 设置;daemon RPC 两种形状都透传。
  - EN: The update toast's release-notes preview never rendered — the workflow ships manifest `notes` as a plain string while the daemon's `updates.check` only accepted a `{zh,en}` object and dropped the string, leaving the version-only card. Notes are now fetched and cached by the Electron main process (`updater-notes` IPC; also carried in the enriched `checkForUpdates` result), no longer gated on daemon connectivity or the `startup.checkUpdate` setting; the daemon RPC passes both shapes through.
- **设置页手动检查更新永远显示"已是最新"**:`updater-check` 只返回 `{enabled:true}`,electron-updater 的 updateInfo 被丢弃 → 有新版本时也判成最新,「检查更新」结果块(notes 片段 + 下载按钮)成了永不渲染的死代码。现在 `checkForUpdates` 用 `updateInfo.version` 与 `app.getVersion()` 比对返回真实 `newer/latest/current`。
  - EN: The settings page's manual update check always reported "Up to date" — `updater-check` returned only `{enabled:true}`, discarding electron-updater's updateInfo, so the richer result block (notes snippet + download button) was dead code. `checkForUpdates` now compares `updateInfo.version` against `app.getVersion()` and returns real `newer/latest/current`.
