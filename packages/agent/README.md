...
Stateful agent with tool execution and event streaming. Built on `@musepi/pi-ai`.

## Installation

```bash
npm install @musepi/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@musepi/pi-agent";
...
```typescript
declare module "@musepi/pi-agent" {
...
```typescript
import { Agent, streamProxy } from "@musepi/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@musepi/pi-agent";
