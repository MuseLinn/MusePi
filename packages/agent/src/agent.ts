...
export interface AgentOptions {
	...
	/**
	 * Freeze gate for every loop this agent runs. Defaults to the
	 * process-global gate (TUI `/pause`); multi-scope hosts pass a per-scope
	 * gate so a pause in one scope never freezes the others.
	 */
	pauseGate?: PauseGate;
}
...
export class Agent {
	...
	/**
	 * The freeze gate this agent's loops consult (per-scope gate from
	 * {@link AgentOptions.pauseGate}, else the process-global one via the
	 * loop's fallback). Side-session builders (security coordinator) read it
	 * to inherit the owning session's pause scope.
	 */
