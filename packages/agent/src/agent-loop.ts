...
import { type CursorExecResolvedCarrier, kCursorExecResolved } from "@musepi/pi-ai/utils/block-symbols";
import {
	...
	signalListLabel,
} from "@musepi/pi-ai/utils/harmony-leak";
import { logger, sanitizeText, structuredCloneJSON } from "@musepi/pi-utils";
...
async function runLoopBody(
	...
	// Freeze gate for this run: the config's per-scope gate when the host
	// provides one (GUI daemon sessions), else the process-global TUI /pause
	// gate. Consulted at both action boundaries below.
	...
	try {
		...
		while (true) {
			...
			while (hasMoreToolCalls || pendingMessages.length > 0) {
				...
				await yieldIfDue();
				// Park at the turn boundary while ANY freeze gate is engaged.
				// The process-global gate (TUI /pause, GUI global pause) is
				// checked first, then this run's scope gate (GUI per-session
				// pause) — the two are orthogonal: releasing the global pause
				// must not release a session that is itself paused, and vice
				// versa. An external abort releases the park so a cancelled
				// run still unwinds while everything else stays frozen.
...
async function executeToolCalls(
	...
	// Freeze gate for this run (config's per-scope gate, else the global one).
	const pauseGate = config.pauseGate ?? agentPauseGate;
	type ToolCallContent = Extract<AssistantMessage["content"][number], { type: "toolCall" }>;
	...
	const runTool = async (record: (typeof records)[number], index: number): Promise<void> => {
		...
		// Park before starting this tool while ANY freeze gate is engaged
		// (process-global first, then this run's scope gate — orthogonal, see
		// the turn-boundary site). Tools already executing are unaffected
		// (pausing never aborts); a batch interrupted mid-pause unwinds via
		// the signal checks below.
