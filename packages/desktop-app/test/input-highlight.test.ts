...
describe("parseHighlightSpans", () => {
	test("line-leading slash command", () => {
		expect(parseHighlightSpans("/security now")).toEqual([{ start: 0, end: 9, kind: "slash" }]);
	});
	...
	test("mid-text slashes stay plain — they are args of the first command, not commands", () => {
		...
		expect(parseHighlightSpans("/security /guided-goal /security")).toEqual([{ start: 0, end: 9, kind: "slash" }]);
	});

	test("escaped // stays plain", () => {
		expect(parseHighlightSpans("//not-a-command")).toEqual([]);
	});

	test("subsequent lines carry their own leading slash", () => {
		expect(parseHighlightSpans("intro\n/btw")).toEqual([{ start: 6, end: 10, kind: "slash" }]);
	});

	test("standalone magic words anywhere; prose boundaries respected", () => {
		expect(parseHighlightSpans("a ultrathink!")).toEqual([{ start: 2, end: 12, kind: "magic" }]);
		expect(parseHighlightSpans("foo.ultrathink")).toEqual([]);
	});

	test("slash wins over a magic word inside the command token", () => {
		expect(parseHighlightSpans("/ultrathink")).toEqual([{ start: 0, end: 11, kind: "slash" }]);
	});

	test("line-leading bang (and !!) wraps the shell head", () => {
		expect(parseHighlightSpans("!ls -la")).toEqual([{ start: 0, end: 3, kind: "bang" }]);
		expect(parseHighlightSpans("!!make test")).toEqual([{ start: 0, end: 6, kind: "bang" }]);
		// "! cmd" executes too (the send path trims the body) — the pill
		// covers the inner whitespace.
		expect(parseHighlightSpans("! cmd")).toEqual([{ start: 0, end: 5, kind: "bang" }]);
	});

	test("mid-text bang stays plain", () => {
		expect(parseHighlightSpans("hi !ls")).toEqual([]);
	});

	test("bang wins over a magic word inside the shell token", () => {
		expect(parseHighlightSpans("!!workflowz")).toEqual([{ start: 0, end: 11, kind: "bang" }]);
	});
});
