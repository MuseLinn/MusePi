...
async function findTarget() {
	for (let i = 0; i < 40; i++) {
		try {
			...
			const targets = (await res.json()) as Array<{ type: string; title: string; url: string; webSocketDebuggerUrl: string; id: string }>;
...
function handleEvent(msg: { method?: string; params?: unknown }): void {
	if (msg.method === "Runtime.exceptionThrown") {
		...
		pageErrors.push(p.exceptionDetails?.exception?.description ?? p.exceptionDetails?.text ?? JSON.stringify(p).slice(0, 400));
...
if (openResult !== "clicked" && openResult !== "already-open") { console.log("ABORT: session not opened"); process.exit(1); }
...
for (let i = 0; i < TICKS; i++) {
	...
	if (i % WHEEL_EVERY === WHEEL_EVERY - 1) {
		await cdp("Input.dispatchMouseEvent", { type: "mouseWheel", x: Math.round(width / 2), y: Math.round(height / 2), deltaX: 0, deltaY: -700 }).catch(() => {});
	}
	await sleep(POLL_MS);
}
...
for (let i = 1; i < ss.length; i++) {
	...
	if (d < -1500) {
		...
		for (const w of near) console.log(`   write t=${w.t} ${w.prev}→${w.v} (sh@write=${w.sh})\n      ${w.stack.slice(0, 280)}`);
