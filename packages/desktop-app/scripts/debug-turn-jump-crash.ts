...
function session(url: string, onEvent?: (msg: { method?: string; params?: any }) => void) {
	...
	const send = (method: string, params: Record<string, unknown> = {}, timeoutMs = 15000) =>
		new Promise(resolve => {
			...
			pending.set(id, (msg: unknown) => { clearTimeout(timer); resolve(msg); });
...
const { ws, ready, send } = session(page.webSocketDebuggerUrl!, (msg: { method?: string; params?: any }) => {
	if (msg.method === "Runtime.exceptionThrown") {
		...
		events.push(`EXCEPTION ${d.exception?.description?.split("\n").slice(0, 6).join(" | ") ?? d.text} @ ${frame ? `${frame.url}:${frame.lineNumber + 1}` : "?"}`);
	} else if (msg.method === "Runtime.consoleAPICalled" && msg.params?.type === "error") {
		const text = (msg.params.args ?? []).map((a: { value?: unknown; description?: unknown }) => a.value ?? a.description ?? "").join(" ").slice(0, 500);
...
}, 30000)) as { __timeout?: boolean; result?: { result?: { value?: unknown } } };
...
const jumpRes = (await send("Runtime.evaluate", {
	...
	returnByValue: true, awaitPromise: true,
}, 15000)) as { __timeout?: boolean; result?: { result?: { value?: unknown } } };
...
for (let i = 0; i < 24; i++) {
	...
	const st = (await send("Runtime.evaluate", {
		...
		returnByValue: true,
	}, 8000)) as { __timeout?: boolean; result?: { result?: { value?: unknown } } };
	if (st.__timeout) { console.log(`t+${(i + 1) * 5}s: eval blocked`); continue; }
