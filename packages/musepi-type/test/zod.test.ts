...
describe("zod-like parsing", () => {
	...
	it("composes defaults before optional determinately like zod", () => {
		// `.default(v).optional()` must not build the indeterminate
		// union([morph, undefined]) that omptype rejects at construction.
		const cell = z.number().int().default(4).optional();
		expect(cell.parse(undefined)).toBe(undefined);
		expect(cell.parse(7)).toBe(7);
		const schema = z.object({
			image: z.string(),
			cell: z.number().int().default(4).optional(),
			backend: z.enum(["vision", "tesseract"]).default("vision").optional(),
		});
		expect(schema.parse({ image: "a" })).toEqual({ image: "a", cell: 4, backend: "vision" });
		expect(schema.parse({ image: "a", cell: undefined })).toEqual({ image: "a", backend: "vision" });
		expect(schema.parse({ image: "a", backend: "tesseract" })).toEqual({ image: "a", cell: 4, backend: "tesseract" });
		expect(schema.safeParse({ image: "a", backend: "nope" }).success).toBe(false);
		// chained composition stays determinate too
		expect(z.number().default(4).optional().default(5).parse(undefined)).toBe(5);
		expect(z.string().default("a").default("b").parse(undefined)).toBe("b");
		expect(z.string().default("x").optional().nullable().parse(null)).toBe(null);
	});
