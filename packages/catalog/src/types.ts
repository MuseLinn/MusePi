...
// Re-exported from @musepi/pi-utils so the whole workspace shares one
// `fetch`-compatible signature (tls-fetch's wrappers produce/accept it).
export type { FetchImpl } from "@musepi/pi-utils";
...
export interface Model<TApi extends Api = Api> {
	...
	reasoning: boolean;
	/**
	 * Native tokenizer family for local token counting, when set
