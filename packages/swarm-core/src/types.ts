...
export type SwarmTimerId = number;

export interface SwarmGlobalState {
	currentSwarm: SwarmState | null;
	activeSessions: Map<string, ActiveSessionEntry> | null;
	cancelPending: boolean;
	cancelTimer: SwarmTimerId | null;
...
export function setResumeResult(id: string, r: { status: string; output?: string }): void {
	resumeResults.set(id, r);
}
export function getResumeResults(): Map<string, { status: string; output?: string }> {
	return resumeResults;
}
export function clearResumeResults(): void {
	resumeResults.clear();
}

export function setCurrentSwarm(s: SwarmState | null): void {
	swarmState.currentSwarm = s;
}
export function setActiveSessions(m: SwarmGlobalState["activeSessions"]): void {
	swarmState.activeSessions = m;
}
export function setCancelPending(v: boolean): void {
	swarmState.cancelPending = v;
}
export function setCancelTimer(t: SwarmGlobalState["cancelTimer"]): void {
	swarmState.cancelTimer = t;
}
export function setSavedSwarmState(s: SwarmGlobalState["savedSwarmState"]): void {
	swarmState.savedSwarmState = s;
}
export function setSwarmCancelled(v: boolean): void {
	swarmState.swarmCancelled = v;
}
export function setGlobalAbortController(c: AbortController | null): void {
	swarmState.globalAbortController = c;
}
