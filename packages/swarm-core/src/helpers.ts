...
export function accumulatedBrailleBar(
	progress: number,
	width: number,
	phase: "pending" | "running" | "done" | "failed" | "aborted",
...
function calculateGridLayoutUncached(count: number, availableWidth: number, availableHeight: number): GridLayout {
	...
	const gapWidth = 2; // CELL_GAP visible width
...
function gradientTextUncached(text: string, fromHex: string, toHex: string, accentBias = 1): string {
	...
	const bias = Number.isFinite(accentBias) ? Math.max(0, accentBias) : 1;
