...
import type { Context, FetchImpl, Model, ModelSpec } from "@musepi/pi-ai/types";
...
describe("resolveOpenAIRequestSetup User-Agent", () => {
	test("sets musepi User-Agent on xAI when none is provided", () => {
...
describe("xAI stream User-Agent", () => {
	test("xAI Responses POST sends musepi User-Agent", async () => {
		...
		expect(captured.userAgent).toMatch(/^musepi\/\d+\.\d+\.\d+$/);
	});

	test("xAI OAuth Responses POST sends musepi User-Agent", async () => {
		const captured = await captureStreamHeaders(
	...
	test("OpenAI Completions POST does not send musepi User-Agent", async () => {
		const captured = await captureStreamHeaders(
