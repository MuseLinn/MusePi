...
import { parseAlibabaTokenPlanCredential } from "@musepi/pi-catalog/wire/alibaba-token-plan";
import { getAgentDbPath, getDbBusyTimeoutMs, logger } from "@musepi/pi-utils";
import type {
	AuthCredential,
	AuthCredentialStore,
...
export function serializeCredential(provider: string, credential: AuthCredential): SerializedCredentialRecord | null {
	if (credential.type === "api_key") {
		// musepi: the user note (multi-account label) rides along with the key.
		const data =
			credential.source === "login"
				? { key: credential.key, source: "login" as const, ...(credential.note ? { note: credential.note } : {}) }
				: { key: credential.key, ...(credential.note ? { note: credential.note } : {}) };
		return {
...
function deserializeCredential(row: AuthRow): AuthCredential | null {
	...
	if (row.credential_type === "api_key") {
		...
		if (typeof data.key === "string") {
			...
			if (source) {
				return note ? { type: "api_key", key: data.key, source, note } : { type: "api_key", key: data.key, source };
			}
			return note ? { type: "api_key", key: data.key, note } : { type: "api_key", key: data.key };
		}
	}
