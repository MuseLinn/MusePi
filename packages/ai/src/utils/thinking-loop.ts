 ...
 * result-awaiting callers (`complete`, `completeSimple`) re-sample it a few
 * times and then let a stubborn loop cook through one unguarded pass. Disable
 * detection with `PI_NO_THINKING_LOOP_GUARD=1`.
 */
import { modelFamilyToken } from "@musepi/pi-catalog/identity";
