...
import { $env, $envExact, extractRetryHint, getAgentDbPath, logger } from "@musepi/pi-utils";
import {
...
export type ApiKeyCredential = {
	...
	key: string;
	source?: "login";
	/** User note distinguishing this credential (multi-account labels). */
	note?: string;
};
...
// `@musepi/pi-ai` entrypoint and the in-module call sites below.
...
function authCredentialEquals(left: AuthCredential, right: AuthCredential): boolean {
	...
	if (left.type === "api_key") {
		return right.type === "api_key" && left.key === right.key && (left.note ?? null) === (right.note ?? null);
	}
...
export class AuthStorage {
	...
	/**
	 * Persist an API-key credential for a provider, replacing any existing
	 * stored credential (/setup parity — import without OAuth). Async to
	 * mirror the OAuth write path; the cache is refreshed immediately so
	 * `has()`/`listStoredCredentials()` reflect the new key.
	 */
	...
	/**
	 * Set (or clear) a user note on one stored credential — the label that
	 * distinguishes accounts in multi-credential UIs. Writes through the
	 * store and refreshes the in-memory row.
	 */
	async setCredentialNote(provider: string, credentialId: number, note: string): Promise<boolean> {
		const rows = this.listStoredCredentials(provider);
		const row = rows.find(r => r.id === credentialId);
		if (!row) return false;
		const updated: AuthCredential = { ...row.credential, note: note.trim() || undefined };
		// Replace the full set so sibling credentials survive.
		const all = rows.map(r => (r.id === credentialId ? updated : r.credential));
		const stored = this.#store.replaceAuthCredentialsForProvider(provider, all);
		this.#setStoredCredentials(
			provider,
			stored.map(entry => ({ id: entry.id, credential: entry.credential })),
		);
