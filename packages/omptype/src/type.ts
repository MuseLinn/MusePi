...
function morphIdentities(ir: IR, identities: unknown[] = [], seen = new Set<IR>()): unknown[] {
	...
	switch (ir.k) {
		case "morph":
			...
			break;
		case "sub": {
			// A sub's schema is an EmbeddableSchema; the step list is an
			// InternalType-only detail (stepped embeddables may omit it).
			const schema = ir.schema as InternalType;
			const steps = schema[kSteps];
			if (steps !== undefined) {
				for (const step of steps) if (step.kind === "pipe") identities.push(step.fn);
			}
