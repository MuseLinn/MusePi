...
/**
 * Compose a stepped schema over an existing schema's input IR with a custom
 * runner. omptype unions are unordered and reject overlapping morph inputs
 * ("an unordered union with overlapping morph inputs is indeterminate"), so
 * `.default(v).optional()` cannot be built as `union([morph, undefined])` —
 * the morph admits undefined and the members' inputs overlap. Keeping the
 * same input IR and moving the undefined branch into the runner stays
 * determinate. `makeType()` rebuilds schema metadata from its meta argument
 * (empty for `raw`), so default metadata is re-applied afterwards.
 */
function composeStepped<Out>(source: Decoratable<Out>, run: (data: unknown) => unknown): Decoratable<Out> {
	const composed: EmbeddableSchema = {
		[IR_BRAND]: true,
		ir: source.ir,
		hasSteps: true,
		hasDefault: source.hasDefault,
		defaultValue: source.defaultValue,
		hasDefaultOutput: source.hasDefaultOutput,
		defaultOutput: source.defaultOutput,
		run,
	};
	const wrapped = type.raw(composed) as Decoratable<Out> & {
		hasSteps: boolean;
		hasDefault: boolean;
	...
	// `raw` builds the type from an empty step list, so the stepped marker and
	// default metadata must be re-applied for the facade's own composition.
	wrapped.hasSteps = true;
	wrapped.hasDefault = source.hasDefault;
	wrapped.defaultValue = source.defaultValue;
	wrapped.hasDefaultOutput = source.hasDefaultOutput;
	wrapped.defaultOutput = source.defaultOutput;
	return wrapped;
}
...
function decorate<Out>(schema: Decoratable<Out>, optional = false): ZodLikeSchema<Out> {
	...
	return Object.assign(schema, {
		...
		optional(): ZodLikeSchema<Out | undefined> & OptionalSchemaMarker {
			// omptype unions are unordered and reject overlapping morph inputs, so a
			// bare `or(undefined)` on a stepped schema whose input already admits
			// undefined (e.g. after `.default(v)`) is indeterminate. Compose a single
			// stepped schema that short-circuits undefined in its own run instead.
			if (schema.hasSteps) {
				const composed = composeStepped(schema, data => (data === undefined ? undefined : schema(data)));
				return decorate(composed, true) as ZodLikeSchema<Out | undefined> & OptionalSchemaMarker;
			}
		...
		default(
			...
			// Same determinism constraint as optional(): widening a stepped schema
			// whose input already admits undefined with `or(undefined)` creates an
			// overlapping-morph union. Compose over the existing input instead.
			if (schema.hasSteps) {
				const factory = typeof value === "function";
				if (factory) {
					const composed = composeStepped(schema, data =>
						data === undefined ? (value as () => DefaultOut)() : schema(data),
					);
					composed.defaultValue = value;
					return decorate(composed) as ZodLikeSchema<DefaultOut>;
				}
				// Mirror `type.default()`: static defaults are validated eagerly.
				const candidate = value as DefaultOut;
				const validated = schema(candidate);
				if (validated instanceof type.errors) {
					throw new OmpTypeError(`ParseError: Default ${validated.summary}`);
				}
				const composed = composeStepped(schema, data => (data === undefined ? candidate : schema(data)));
				composed.defaultValue = value;
				composed.defaultOutput = validated;
				composed.hasDefaultOutput = true;
				return decorate(composed) as ZodLikeSchema<DefaultOut>;
			}
			const widened = schema.or(type.raw("undefined")) as Decoratable<Out | undefined>;
