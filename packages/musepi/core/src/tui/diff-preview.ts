// ============================================================
// TUI — Clustered diff preview (pure, no pi imports).
//
// Ported from Kimi Code's
// apps/kimi-code/src/tui/components/media/diff-preview.ts: LCS DP
// line alignment, trailing-delete suppression while the new text is
// still streaming (isIncomplete — prevents the "red wall" flash), and
// cluster-based rendering that shows changed regions with a few
// context lines and elides long unchanged runs with a
// `… N unchanged lines …` separator.
//
// Difference from the original: colors are injected as a DiffStyles
// value instead of reading chalk + the kimi theme directly, so core
// stays dependency-free. The adapter (coding-agent TUI layer) passes
// themed styles; tests and headless callers use makePlainStyles().
//
// Upstream sync: verified current with MoonshotAI/kimi-code main @
// c5b6103b (2026-07-20). Re-check with:
...
/** Identity styles — no color. Default for headless callers and tests. */
export function makePlainStyles(): DiffStyles {
  const id = (s: string) => s;
  return { add: id, del: id, addBold: id, delBold: id, gutter: id, meta: id };
}
...
export function renderDiffLines(
  ...
  newStart?: number,
  maxLines?: number,
  styles: DiffStyles = makePlainStyles(),
): string[] {
  const s = styles;
  ...
  if (hidden > 0) {
    output.push(
      s.meta(`     … ${String(hidden)} more change${hidden > 1 ? "s" : ""} hidden (ctrl+o to expand)`),
    );
  }
...
export function renderDiffLinesClustered(
  ...
  const s = opts.styles ?? makePlainStyles();
  ...
  if (truncated) {
    ...
    if (hidden > 0) {
      ...
      output.push(
        s.meta(`     … ${String(hidden)} more change${hidden > 1 ? "s" : ""} hidden (${hint} to expand)`),
      );
    }
  }
