...
/**
 * Native addon loader for `@musepi/pi-natives`.
 *
...
function resolveLeafPackageDir(platformTag) {
	try {
		...
		return path.dirname(require_.resolve(`@musepi/pi-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * `node_modules/@musepi/pi-natives/native/`, leaving an old binary next to a
...
export function validateLoadedBindings(ctx, bindings, candidate) {
	...
	if (residentSentinel && diskHasExpectedSentinel) {
		...
		throw new Error(
			`Loaded ${candidate}, which exposes the @musepi/pi-natives@${residentVersion} version ` +
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @musepi/pi-natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @musepi/pi-natives\n" +
...
export function initLoaderContext(overrides = {}) {
	...
	// `import.meta.dir` is a Node ≥24 alias; Bun and Electron's Node 22 only
	// expose `import.meta.dirname` (Electron's own Node lacks the alias).
	const metaDir = /** @type {string | undefined} */ (import.meta.dir ?? import.meta.dirname);
