...
function writeRecord(stem: string, en: string, zh: string): void {
	const p = i18nPath(stem);
	const body =
		`# Consistency record for ${basename(stem)}.md / ${basename(stem)}.zh-CN.md\n` +
...
function checkStem(stem: string): Row {
	...
	if (enRec !== enHash || zhRec !== zhHash) {
		return {
			stem: rel,
			status: "out-of-sync",
			detail: `record ${enRec?.slice(0, 8) ?? "-"}/${zhRec?.slice(0, 8) ?? "-"} vs ${enHash.slice(0, 8)}/${zhHash.slice(0, 8)}`,
		};
	}
	if (!hasSwitcher(en, "en"))
		return { stem: rel, status: "missing-switcher", detail: "English side lacks 'English | [中文](…zh-CN.md)'" };
	if (!hasSwitcher(zh, "zh"))
		return { stem: rel, status: "missing-switcher", detail: "Chinese side lacks '[English](…md) | 中文'" };
...
const targets = named.length > 0 ? namedStems.map(s => (allStems.includes(s) ? s : `docs/${s}`)) : allStems;
