 ...
 *
 * The default mode publishes public JS packages and the `@musepi/pi-natives`
...
export const packages: PublishPackage[] = [
	...
	{ dir: "packages/coding-agent", kind: "typescript", publishBin: { musepi: "dist/cli.js" } },
];
...
function buildNativeOptionalDependencies(version: string): JsonObject {
	...
	for (const target of LEAF_TARGETS) {
		optionalDependencies[`@musepi/pi-natives-${target.tag}`] = version;
	}
...
async function packAndPublish(dir: string, name: string): Promise<void> {
	...
	const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "musepi-pack-"));
	try {
