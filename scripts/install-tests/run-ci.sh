...
# 3. Pack the remaining workspace packages (natives core and coding-agent
   #    handled separately). `client-core` is private but still packed here so its
#    prepack build and tarball file list stay release-safe.
for pkg in utils wire hashline catalog ai mnemopi snapcompact agent tui stats client-core musepi-type collab-proto sdk tool-select; do
   (
...
desktop_web_tgz="$(find_tarball "$TARBALL_DIR"/musepi-client-core-*.tgz)"
...
(
   ...
   node -e "
		...
		pkg.overrides = {
			'@musepi/pi-utils': '$utils_tgz',
			'@musepi/pi-wire': '$wire_tgz',
			'@musepi/pi-natives': '$natives_tgz',
			'@musepi/pi-natives-$host_tag': '$natives_leaf_tgz',
			'@musepi/hashline': '$hashline_tgz',
			'@musepi/pi-ai': '$ai_tgz',
			'@musepi/pi-catalog': '$catalog_tgz',
			'@musepi/pi-mnemopi': '$mnemopi_tgz',
			...
			'@musepi/client-core': '$desktop_web_tgz',
   ...
   leaf_dir="node_modules/@musepi/pi-natives-$host_tag"
   [ -d "$leaf_dir" ] || {
      echo "Platform leaf package not installed: $leaf_dir"
      exit 1
   }
   wire_proto="$(bun -e 'import { COLLAB_PROTO } from "@musepi/pi-wire"; process.stdout.write(String(COLLAB_PROTO));')"
   [ "$wire_proto" = "3" ] || {
      echo "Unexpected @musepi/pi-wire COLLAB_PROTO: $wire_proto"
      exit 1
   }
   omptype_probe="$(bun -e '
   ...
   [ -f "node_modules/@musepi/client-core/dist/index.html" ] || {
