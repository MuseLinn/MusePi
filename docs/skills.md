...
- `description` is required for:
  - native `.musepi` provider skill discovery (`requireDescription: true`)
...
7. `omp-managed` (priority 5) — auto-learn skills under `~/.musepi/agent/managed-skills`, registered in `src/discovery/builtin.ts` and discovered unconditionally (only writing/nudging is gated by `autolearn.enabled`); always defers to a same-named authored skill
