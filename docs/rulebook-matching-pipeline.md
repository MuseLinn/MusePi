...
- project: `<cwd>/.musepi/rules/*.{md,mdc}` when the cwd `.musepi` directory exists
- user: `~/.musepi/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.musepi/agent/RULES.md`
- sticky project rule: nearest ancestor `.musepi/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.musepi/rules`, user `~/.musepi/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
