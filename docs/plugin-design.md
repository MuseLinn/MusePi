...
> 2026-08-25 核对（`docs/audit-plugin-seams.md`）：P0（8/8）与 P1（veto/observe）已落地并
> 超前于本文件;P2 ◐（命令/快捷键 ✅、状态行 ❌）;P3 ❌（主题 token/通知通道）;P4 ◐（定时器 ✅、服务域 ❌）。
> 本文件定义核心边界、接缝清单、可逆卸载契约与阶段路线,供后续实现对齐。

## 0. 定位：介于 pi 与 dsh 之间

> **2026-08-25 用户拍板**：MusePi 的最终形态是 **pi 的组件完备性 + dsh 的扩展生态** 的折中——
> 内置组件（工具/GUI 面板/TUI 命令/看板 widget）开箱即用、功能完整（pi 侧价值），
> 同时扩展体系按 dsh 的 Cordis 思路演进：挂载式插件、事件驱动、可逆卸载（dsh 侧价值）。
> 取舍底线（不可逾越）：
> - **不引入 cordis.patch.yml 补丁面**（核心源码补丁）——插件只能经白名单接缝扩展。
> - **不引入改写型决策事件**（拦截/改写输入、rewind、压缩）——决策点只允许 pass/veto/inject。
> - **卸载必可逆**——快照先行 + 回滚 = 恢复 + 重载，永不触碰核心源码。
...
| 阶段 | 内容 | 验收 | 状态(2026-08-25 核对) |
|---|---|---|---|
| P0(现状基线) | GUI slot + extension 装载/回滚/沙箱已可用 | E2E 已覆盖 | ✅ 达成(8/8,强于文档) |
