...
| 9 | 会话分享附件校验 | 小-中 | 无 | 第二批 | ✅ 已落地（2026-09-24） |
...
> ✅ **已落地（2026-09-24）**：发布前 `computeAttachmentStats` 统计 + 超 5 MB 抛 `ShareTooLargeError`（含 `canRetryWithStrippedAttachments`）；`stripAttachments: true` 走 `sealToFit` 显式剥离步（fileMention 内容换占位文本、图片换占位），结果带 `strippedAttachments` 计数；剥离在脱敏之后。测试 `packages/coding-agent/test/share-attachments.test.ts`（9 例，含脱敏顺序硬约束）。
