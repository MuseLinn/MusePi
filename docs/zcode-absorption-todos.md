...
| # | 项 | 规模 | 前置 | 建议批次 |
|---|---|---|---|---|
| 1 | 闲时任务（自然语言创建 + 聊天内卡片） | 中 | 无 | 第一批 |
| 2 | @ 引用支持中文搜索 | 小 | 无 | 第一批 |
| 3 | 问答卡换行输入（>5 行内滚） | 小 | 无 | 第一批 |
| 5 | 子智能体可选模型与供应商一致性 | 小-中 | 无（先审计） | 第一批 |
| 8 | 拖动指示线视觉重量 | 小 | **需确认指哪一处** | 第一批 |
| 9 | 会话分享附件校验 | 小-中 | 无 | 第二批 |
| 4 | 输入框「+」菜单扩展 | 中 | 无 | 第二批 |
| 7 | 高 DPI / 应用缩放点击坐标 | 中 | 无 | 第三批 |
| 6 | Windows 截图链路加固 | 大 | 无 | 第三批（拆 2-3 PR） |
