...
2. Custom provider and model entries from `~/.musepi/agent/models.yml`.
...
When a provider needs an API key, `musepi` resolves it in this order (first match wins):
...
Stored credentials live in the auth store at `~/.musepi/agent/agent.db` for local auth, or in the configured auth-broker snapshot when running in broker mode. (`PI_CODING_AGENT_DIR` relocates the `~/.musepi/agent` base, and the auth store moves with it.)
...
`musepi` eagerly loads `.env` files into the process environment before any provider lookup. It reads four files and, for each variable, the **first** source that defines it wins. Effective precedence, high to low:

1. The process environment inherited by `musepi` (already-set variables always win).
2. `<cwd>/.env`
3. `~/.musepi/agent/.env`
4. `~/.musepi/.env`
5. `~/.env`

A variable already present in the process environment is never overwritten by a `.env` file. Among the files, a value set in `<cwd>/.env` wins over `~/.musepi/agent/.env`, which wins over `~/.musepi/.env`, which wins over `~/.env`. So a shell-exported `OPENAI_API_KEY` beats every `.env` file, and a project's `<cwd>/.env` beats your home `~/.env`.
...
Project settings live in `<project>/.musepi/config.yml`. Use them when one repository must allow or hide a different provider set than your global default:
...
Custom providers live in `~/.musepi/agent/models.yml` under `providers:`. A provider ID defined there participates in the same selection, credential resolution, and `disabledProviders` rules as built-in providers.
