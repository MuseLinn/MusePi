...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@musepi/pi-coding-agent`
  - `@mariozechner/pi-agent-core` → `@musepi/pi-agent-core`
  - `@mariozechner/pi-tui` → `@musepi/pi-tui`
  - `@mariozechner/pi-ai` → `@musepi/pi-ai`
  - `@mariozechner/pi-utils` → `@musepi/pi-utils`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/pi-coding-agent` → `@musepi/pi-coding-agent`, and so on).
- The bare `typebox` package is not an `@musepi/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
Do not add new runtime asset copy steps. Keep assets in repo and prefer Bun embeds/imports; preserve existing explicit generation workflows such as `packages/coding-agent/src/export/html/tool-views.generated.js` (built from guest-client sources via `bun run gen:tool-views`).
...
- In `packages/coding-agent`, use `logger` from `@musepi/pi-utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@musepi/pi-natives` exports generated native bindings from `packages/natives/native`.
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@musepi/pi-natives`
