...
- **Core directories**:
  - `leafPackageDir`: directory of the platform leaf package, resolved via `require.resolve("@musepi/pi-natives-<tag>/package.json")`; `null` when no leaf is installed (e.g. local dev) and forced to `null` in compiled-binary mode.
...
- **Natives cache root** (`getNativesDir()`):
  - if `$XDG_DATA_HOME/musepi` exists, `$XDG_DATA_HOME/musepi/natives`;
  - otherwise `~/.musepi/natives`.
...
- release sentinel mismatch details when a loadable `.node` belongs to another `@musepi/pi-natives` version.

### Non-compiled startup failures

Normal package/runtime diagnostics include:

- reinstall hint (`bun install @musepi/pi-natives`),
