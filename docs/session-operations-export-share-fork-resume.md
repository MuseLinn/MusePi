...
- Tool calls render through the `<omp-tool-view>` web component — the React per-tool renderers shared with guest-client (`packages/client-core/src/tool-render/`), prebuilt into `src/export/html/tool-views.generated.js` by `bun run gen:tool-views`.
