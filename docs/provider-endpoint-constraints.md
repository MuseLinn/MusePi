...
- [Providers](./providers.html) — provider availability, credentials, custom providers
- [Model and Provider Configuration](./models.html) — `models.yml`, routing, and compat fields
- [Provider streaming internals](./provider-streaming-internals.html) — stream event normalization
- [Provider compat reference](./provider-compat-reference.html) — every compat flag, reasoning levels, tool handling per provider
- [Provider quirks](./provider-quirks.html) — per-provider special casings, stream behavior, auth/usage, catalog handling
- [Adding a provider](./adding-a-provider.html) — catalog/auth wiring for a new provider
