...
- **Partial JSON Throttling**: `parseStreamingJsonThrottled` (from `@musepi/pi-utils`) throttles incremental JSON parsing during tool argument streaming in `streamOpenAICompletionsOnce` to avoid high CPU overhead.
