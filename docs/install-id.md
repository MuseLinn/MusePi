...
Exported from `@musepi/pi-utils` (`packages/utils/src/dirs.ts`):
...
- Path: `<base-config-root>/install-id` — i.e. `~/.musepi/install-id` by default, respecting `PI_CONFIG_DIR`. Resolved against the base config root (`getBaseConfigRoot()`) regardless of the active profile, so every profile on a host shares one install ID (install identity is per-install, not per-profile).
- Format: a single UUID line (trailing `\n`).
- Permissions: file is created with mode `0o600`.
- Lifecycle: independent of `~/.musepi/agent/`. Wiping agent state (sessions, settings, DB) does NOT regenerate the install ID; only deleting the `install-id` file itself does.
...
- [environment-variables.md](environment-variables.html) — `PI_CONFIG_DIR` controls where `install-id` lives.
- [config-usage.md](config-usage.html) — broader config-root layout.
