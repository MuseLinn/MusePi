...
```
musepi plugin marketplace add ./docs/skills/examples/mini-marketplace
musepi plugin install my-plugin@example-marketplace
```
...
Published and local marketplaces use the same catalog location. musepi loads `.omp-plugin/marketplace.json` first and falls back to `.claude-plugin/marketplace.json` (the Claude Code-compatible path this example ships) inside the marketplace root. Point `/marketplace add` at this folder to load the example.
