...
description: Use when creating a new musepi extension. Covers ExtensionAPI, factory signature, tool/command/event registration, and local-dev testing.
---
...
That is a working extension. Drop it into `~/.musepi/agent/extensions/hello.ts` and restart musepi to see the notification.
...
musepi loads extension modules from these sources:

1. Native `.musepi` locations discovered through the capability system:
   - `<cwd>/.musepi/extensions/`
   - `~/.musepi/agent/extensions/`
   - legacy extension paths listed in `.musepi/settings.json#extensions` or `~/.musepi/agent/settings.json#extensions`
2. Installed plugins under `~/.musepi/plugins/node_modules` (`musepi plugin install` npm/git specs, or `musepi plugin link`) via their `omp.extensions`/`pi.extensions` manifests. Marketplace cache installs do not feed extension modules — they surface skills/commands/hooks/tools/MCP only.
3. Explicit configured paths passed by the CLI (`musepi --extension ./my-ext.ts`, also `-e`; `--hook` is treated as an alias) and by the `extensions:` setting in config.

The runtime de-duplicates by resolved absolute path — first seen wins.

When a path points to a directory, musepi resolves the entry point in this order:

1. `package.json` with `omp.extensions` (or legacy `pi.extensions`) field
2. `index.ts`
3. `index.js`

When scanning an `extensions/` directory, musepi also loads direct `*.ts`/`*.js` files and one-level subdirectories that have `index.ts`, `index.js`, or a manifest.
...
| Legacy hook module already exists | **Hook** (`HookAPI` from `@musepi/pi-coding-agent/extensibility/hooks`) |
...
musepi writes structured logs to a rotating file under `~/.musepi/logs/` (debug level is always on; nothing is written to the console, which would corrupt the TUI). Tail today's log to see extension load diagnostics:

```
tail -f ~/.musepi/logs/musepi.$(date +%F).log
```
