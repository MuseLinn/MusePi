...
description: Use when creating a new musepi hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
export default function myHook(musepi: HookAPI): void {
  musepi.on("tool_call", async (event, ctx) => {
...
```ts
musepi.on("tool_result", async (event, ctx) => {
...
```ts
musepi.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(musepi: HookAPI): void {
  musepi.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(musepi: HookAPI): void {
  musepi.on("tool_result", async (event) => {
...
export default function contextFilter(musepi: HookAPI): void {
  musepi.on("context", async (event) => {
