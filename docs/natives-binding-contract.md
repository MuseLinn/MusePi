...
This document defines the JS/TS contract between `@musepi/pi-natives` callers and the loaded N-API addon.
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@musepi/pi-natives`.
