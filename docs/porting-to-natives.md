...
- `packages/natives/package.json` exposes only the package root (`@musepi/pi-natives`) as the import surface. At publish time the binaries are split out: the core ships the loader only (no `.node`), and each platform's `.node` is published as an optional-dependency leaf package `@musepi/pi-natives-<tag>` (`scripts/ci-release-publish.ts` + `packages/natives/scripts/gen-npm-packages.ts`). This is transparent to importers — you still `import` from `@musepi/pi-natives`.
...
- Import the new export directly from `@musepi/pi-natives`.
