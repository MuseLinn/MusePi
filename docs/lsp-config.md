...
| 3           | User config dirs: `~/.musepi/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.musepi/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.musepi/agent/lsp.json`
- Project-specific overrides → `<project>/.musepi/lsp.json`
...
Place the override in `<project>/.musepi/lsp.json`:
...
The user-level config in `~/.musepi/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
