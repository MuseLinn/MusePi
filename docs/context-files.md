...
- **Walk-up to the repository root.** Discovery starts in the current working directory and climbs through each ancestor up to the repository root, stopping at the first ancestor that has a usable `.musepi/` directory. The *nearest* match wins; ancestors above it are not loaded as native context.
- **The `.musepi/` directory must be non-empty.** An empty `.musepi/` directory is skipped during the walk-up, so the search continues to the next ancestor. An empty `AGENTS.md` or `RULES.md` file contributes nothing.
...
- The native context file is `repo/packages/api/.musepi/AGENTS.md` (the nearest one). `repo/.musepi/AGENTS.md` is **not** also included.
- The project sticky rule is `repo/packages/api/.musepi/RULES.md` if present; otherwise the walk-up continues and `repo/.musepi/RULES.md` is used.
...
- **One user context file** is kept across all providers. Because `native` has the highest priority, `~/.musepi/agent/AGENTS.md` shadows every other user-level context file.
...
- If you add `repo/packages/api/.musepi/AGENTS.md`, `native` (priority 100) wins depth 0 outright, shadowing both lower-priority files.
...
- Native project context must live at `.musepi/AGENTS.md`, and the `.musepi/` directory must be non-empty; an empty `.musepi/` is skipped and the walk-up continues to the next ancestor.
