...
- **右上角悬浮状态区是一张卡内的分区,不是三张卡**(`StatusCards.tsx` + gui-chat.css):git/子智能体/待办此前是三张独立磨砂卡纵向堆叠,折叠控制是悬在堆叠外左上角的小圆点(`top:-7px;left:-14px`),折叠态是纯静态胶囊。现对齐 ZCode 状态面板:单卡 + 可折叠分区(弱化标题行、chevron 悬停浮现 —— 纯 CSS),内容区限高 `min(64dvh, 32rem)` 内部滚动,收起控制钉在卡内右上角(absolute 定位,不随内容滚走;首个分区头用右内边距给它留位)。Git 分区行序对齐 ZCode:更改(+N/−M 折叠明细) → 分支切换 → 提交或推送。胶囊⇄面板走 HeightMorph 标准、同一挂载元素(高度按差值 240→480ms + 160ms 内容淡入;圆角/背景/阴影随类翻转 240ms 缓动 —— 宽度直接切换,ZCode 同款取舍);胶囊首图标悬停交叉淡化为展开字形(纯 CSS 切换),主信号段 = 工作区更改 + 差异统计(干净时回退分支名 + ↑↓),并保留智能体/待办段。ZCode 的 ⋯ 展开策略菜单没有对应物(本地只有 `musepi-gui-status-cards` 一个持久化布尔),未移植。窄聊天护栏:视口相对的 `max-width` 放在 `.gui-status-cards` 包装层 —— 收缩包裹的卡片上写百分比会按自身内容宽解析,等于没有约束。
