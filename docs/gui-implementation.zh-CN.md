...
- **会话地图不是线性的 transcript**(2026-09-12):`SessionTreeCanvas` 自带一套链段折叠(超过 24 个节点折成标着「链段折叠」的 `.stc-fold` 胶囊)—— 把 transcript 的语义漏进了一个「以展示每个节点为目的」的视图。现端到端删除折叠(`layoutTree` 不再有折叠分支、没有胶囊 CSS、没有右键项,两个语言域里只服务该胶囊的 6 个 key 一并移除)。节点改为 208×76 内容卡:头部(类型图标、取自 transcript 域的角色/工具标题、分支数、判定、时间)+ 两行截断摘要;悬停操作浮到右上角,不再常驻占据那 168px 条带里的 ~66px(正是它把标签挤成省略号的原因);大树以 `MIN_FIT_SCALE` 0.75 平移浏览,而不是缩到不可读。
