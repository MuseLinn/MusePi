...
- Key collaborators:
  ...
  - `@musepi/pi-natives` (`crates/pi-iso`) — isolation PAL: `isoResolve` / `isoStart` / `isoStop` backend resolution and fallback.
...
- Agent discovery precedence is first-wins by exact name: project `.musepi` agents dir before the user `.musepi` dir (task agents only load from `.musepi` roots; `.claude`/`.codex`/`.gemini` agent dirs are skipped), Claude plugin agent dirs after config dirs, bundled agents last. Create-time discovery is memoized per cwd for the prompt description; execution-time discovery stays fresh.
